...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.1"
  ...
  <xsl:template name="mshelpTitles">
	...
	<xsl:template name="oldIndexMetadata">
      ...
      <xsl:when test="($group='member') and (starts-with($key,'Overload:') or not(/document/reference/memberdata/@overload='true'))">
        <!-- member -->
	...
	<xsl:template name="helpMetadata">
      ...
      <!-- type overview and member list pages get type and namespace.type keywords -->
			<xsl:when test="$group='type' or ($group='members' and $subgroup='members')">
 ...
 -->
  ...
  <xsl:template name="indexMetadata">
    <xsl:choose>
      ...
      <xsl:when test="$group='members' and $subgroup='members'">
  ...
  <xsl:template name="qualifiedTextNames">
    <xsl:choose>
      ...
      <xsl:when test="boolean(apidata/@group='member') and boolean(containers/type)">
      ...
      <xsl:when test="containers/namespace/apidata/@name">
        <xsl:variable name="left">
