...
namespace Axiom.SceneManagers.IPLSceneManager.Query
{
	...
	/// </summary>
	public class IPLIntersectionSceneQuery: DefaultIntersectionSceneQuery
	{
		/// <summary>
		///		Constructor
		/// </summary>
		/// <param name="creator">SceneManage that created this query</param>
		public IPLIntersectionSceneQuery( SceneManager creator ) : base( creator )
		{
		...
		public override void Execute( IIntersectionSceneQueryListener listener )
		{
		    ...
		    IPLSceneManager sceneMgr = (IPLSceneManager)( this.creator );

            for(int i = 0; i < sceneMgr.EntityList.Count; i++) 
			{
                Entity entityA = sceneMgr.EntityList[i];
