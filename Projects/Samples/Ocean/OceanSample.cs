...
namespace Axiom.Samples.Ocean
{
	...
	public class OceanSample : SdkSample
	{
		...
		protected void SetupGUI()
		{
			...
			btn.CursorPressed += btn_CursorPressed;
			for ( int i = 0; i < ControlsPerPage; i++ )
			{
				this.shaderControls[ i ] = TrayManager.CreateThickSlider( TrayLocation.TopRight, "ShaderControlSlider" + i, "Control", 256, 80, 0, 1, 100 );
			}
		...
		private void selectMenu_SelectedIndexChanged( SelectMenu sender )
		{
			if ( sender != null )
			{
				...
				this.activeMaterial = (Material)MaterialManager.Instance.GetByName( this.materialControlsContainer[ this.currentMaterial ].MaterialName );
					...
					if ( this.activePass != null )
					{
						...
						for ( int i = 0; i < ControlsPerPage; i++ )
						{
							...
							if ( i < numControls )
							{
								...
								ShaderControl activeShaderDef = this.materialControlsContainer[ this.currentMaterial ].GetShaderControl( controlIndex );
								...
								shaderControlSlider.SliderMoved += shaderControlSlider_SliderMoved;
								...
								switch ( activeShaderDef.Type )
								{
									...
									case ShaderType.GpuFragment:
										{
											GpuProgramParameters activeParameters = ( activeShaderDef.Type == ShaderType.GpuVertex ) ? this.activeVertexParameters : this.activeFragmentParameters;
		...
		private void shaderControlSlider_SliderMoved( object sender, Slider slider )
		{
			...
			Contract.Requires( sliderIndex != -1 );
