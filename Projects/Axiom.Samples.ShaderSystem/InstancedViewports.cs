...
{
    class InstancedViewports : SubRenderState
    {
        public static string SGXType = "SGX_InstancedViewports";
        static string SGXLibInstancedViewports = "SampleLib_InstancedViewports";
        static string SGXFuncInstancedViewportsTransform = "SGX_InstancedViewportsTransform";
        static string SGXFuncInstancedViewportsDiscardOutOfBounds = "SGX_InstancedViewportsDiscardOutOfBounds";
        Parameter vsInPosition;
        Parameter vsOriginalOutPositionProjectiveSpace;
        Parameter vsOutPositionProjectiveSpace;
        Parameter psInPositionProjectiveSpace;
        UniformParameter vsInMonitorsCount;
        UniformParameter psInMonitorsCount;
        Parameter vsInMonitorIndex;
        Parameter vsOutMonitorIndex;
        Parameter psInMonitorIndex;

        Parameter vsInViewportOffsetMatrixR0;
        Parameter vsInViewportOffsetMatrixR1;
        Parameter vsInViewportOffsetMatrixR2;
        Parameter vsInViewportOffsetMatrixR3;

        UniformParameter worldViewMatrix;
        UniformParameter projectionMatrix;

        Vector2 monitorsCount;
        bool monitorsCountChanged;
        ...
        protected override bool ResolveParameters(Axiom.Components.RTShaderSystem.ProgramSet programSet)
        {
            ...
            vsInPosition = vsMain.ResolveInputParameter(Parameter.SemanticType.Position, 0, Parameter.ContentType.PositionObjectSpace, Graphics.GpuProgramParameters.GpuConstantType.Float4);
            if (vsInPosition == null)
            {
                return false;
            }

            vsOriginalOutPositionProjectiveSpace = vsMain.ResolveOutputParameter(Parameter.SemanticType.Position, 0, Parameter.ContentType.PositionProjectiveSpace, Graphics.GpuProgramParameters.GpuConstantType.Float4);
            if (vsOriginalOutPositionProjectiveSpace == null)
                return false;

            Parameter.ContentType positionProjectiveSpaceAsTexcoord = (Parameter.ContentType)(Parameter.ContentType.CustomContentBegin + 1);
            vsOutPositionProjectiveSpace = vsMain.ResolveOutputParameter(Parameter.SemanticType.TextureCoordinates, -1, positionProjectiveSpaceAsTexcoord, Graphics.GpuProgramParameters.GpuConstantType.Float4);
            if (vsOutPositionProjectiveSpace == null)
                return false;

            //Resolve ps input position in projective space
            psInPositionProjectiveSpace = psMain.ResolveInputParameter(Parameter.SemanticType.TextureCoordinates, vsOutPositionProjectiveSpace.Index, vsOutPositionProjectiveSpace.Content, Graphics.GpuProgramParameters.GpuConstantType.Float4);
            if (psInPositionProjectiveSpace == null)
                return false;

            //Resolve vertex shader uniform monitors count
            vsInMonitorsCount = vsProgram.ResolveParameter(Graphics.GpuProgramParameters.GpuConstantType.Float2, -1, Graphics.GpuProgramParameters.GpuParamVariability.Global, "monitorsCount");
            if (vsInMonitorsCount == null)
                return false;

            //Resolve pixel shader uniform monitors count
            psInMonitorsCount = psProgram.ResolveParameter(Graphics.GpuProgramParameters.GpuConstantType.Float2, -1, Graphics.GpuProgramParameters.GpuParamVariability.Global, "monitorsCount");
            if (psInMonitorsCount == null)
                return false;

            //Resolve the current world & view matrices concatenated
            worldViewMatrix = vsProgram.ResolveAutoParameterInt(Graphics.GpuProgramParameters.AutoConstantType.WorldViewMatrix, 0);
            if (worldViewMatrix == null)
                return false;

            //Resolve the current projection matrix
            projectionMatrix = vsProgram.ResolveAutoParameterInt(Graphics.GpuProgramParameters.AutoConstantType.ProjectionMatrix, 0);
            if (projectionMatrix == null)
                return false;

            var monitorIndex = Parameter.ContentType.TextureCoordinate3;
            //Resolve vertex shader monitor index
            vsInMonitorIndex = vsMain.ResolveInputParameter(Parameter.SemanticType.TextureCoordinates, 3, monitorIndex, Graphics.GpuProgramParameters.GpuConstantType.Float4);
            ...
            vsInViewportOffsetMatrixR0 = vsMain.ResolveInputParameter(Parameter.SemanticType.TextureCoordinates, 4, matrixR0, Graphics.GpuProgramParameters.GpuConstantType.Float4);
            if (vsInViewportOffsetMatrixR0 == null)
                return false;
            vsInViewportOffsetMatrixR1 = vsMain.ResolveInputParameter(Parameter.SemanticType.TextureCoordinates, 4, matrixR1, Graphics.GpuProgramParameters.GpuConstantType.Float4);
            if (vsInViewportOffsetMatrixR1 == null)
                return false;
            vsInViewportOffsetMatrixR2 = vsMain.ResolveInputParameter(Parameter.SemanticType.TextureCoordinates, 4, matrixR2, Graphics.GpuProgramParameters.GpuConstantType.Float4);
            if (vsInViewportOffsetMatrixR2 == null)
                return false;
            vsInViewportOffsetMatrixR3 = vsMain.ResolveInputParameter(Parameter.SemanticType.TextureCoordinates, 4, matrixR3, Graphics.GpuProgramParameters.GpuConstantType.Float4);
            if (vsInViewportOffsetMatrixR3 == null)
                return false;

            vsOutMonitorIndex = vsMain.ResolveOutputParameter(Parameter.SemanticType.TextureCoordinates, -1, monitorIndex, Graphics.GpuProgramParameters.GpuConstantType.Float4);
        ...
        {
            get { return (int)FFPRenderState.FFPShaderStage.PostProcess + 1; }
        }
    }
    class InstancedViewportsFactory : SubRenderStateFactory
    {
