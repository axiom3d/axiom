...
{
    class ReflectionMap : SubRenderState
    {
        public static string SGXType = "SGX_ReflectionMap";
        static string SGXLibReflectionMap = "SampleLib_ReflectionMap";
        static string SGXFuncApplyReflectionMap = "SGX_ApplyReflectionMap";
        string reflectionMapTextureName;
        string maskMapTextureName;
        int maskMapSamplerIndex;
        int reflectionMapSamplerIndex;
        Real reflectionPowerValue;
        bool reflectionPowerChanged;
        TextureType reflectionMapType;
        UniformParameter maskMapSampler;
        UniformParameter reflectionMapSampler;
        UniformParameter reflectionPower;
        Parameter vsInMaskTexcoord;
        Parameter vsOutMaskTexcoord;
        Parameter vsOutReflectionTexcoord;
        Parameter psInMaskTexcoord;
        Parameter psInReflectionTexcoord;
        UniformParameter worldMatrix, worldITMatrix,
            viewMatrix;
        ...
        protected override bool ResolveParameters(ProgramSet programSet)
        {
            ...
            vsInMaskTexcoord = vsMain.ResolveInputParameter(Parameter.SemanticType.TextureCoordinates, 0, Parameter.ContentType.TextureCoordinate0, GpuProgramParameters.GpuConstantType.Float2);
            ...
            //Resolve vs output mask texture coordinates.
            vsOutMaskTexcoord = vsMain.ResolveOutputParameter(Parameter.SemanticType.TextureCoordinates, -1, vsInMaskTexcoord.Content, GpuProgramParameters.GpuConstantType.Float2);
            if (vsInMaskTexcoord == null)
                return false;

            //Resolve ps input mask texture coordinates.
            psInMaskTexcoord = psMain.ResolveInputParameter(Parameter.SemanticType.TextureCoordinates, vsOutMaskTexcoord.Index, vsOutMaskTexcoord.Content, GpuProgramParameters.GpuConstantType.Float2);

            //Resolve vs output reflection texture coordinates
            vsOutReflectionTexcoord = vsMain.ResolveOutputParameter(Parameter.SemanticType.TextureCoordinates, -1, Parameter.ContentType.Unknown, (reflectionMapType == TextureType.TwoD) ? GpuProgramParameters.GpuConstantType.Float2 : GpuProgramParameters.GpuConstantType.Float3);
            if (vsOutReflectionTexcoord == null)
                return false;

            //Resolve ps input reflection texture coordinates.
            psInReflectionTexcoord = psMain.ResolveInputParameter(Parameter.SemanticType.TextureCoordinates, vsOutReflectionTexcoord.Index, vsOutReflectionTexcoord.Content, vsOutReflectionTexcoord.Type);
            ...
            maskMapSampler = psProgram.ResolveParameter(GpuProgramParameters.GpuConstantType.Sampler2D, maskMapSamplerIndex, GpuProgramParameters.GpuParamVariability.Global, "mask_sampler");
            if (maskMapSampler == null)
                return false;

            reflectionMapSampler = psProgram.ResolveParameter((reflectionMapType == TextureType.TwoD) ? GpuProgramParameters.GpuConstantType.Sampler2D : GpuProgramParameters.GpuConstantType.SamplerCube,
                reflectionMapSamplerIndex, GpuProgramParameters.GpuParamVariability.Global, "reflection_texture");
            if (reflectionMapSampler == null)
                return false;

            reflectionPower = psProgram.ResolveParameter(GpuProgramParameters.GpuConstantType.Float1, -1, GpuProgramParameters.GpuParamVariability.Global, "reflection_power");
        ...
        private bool AddVsInvocations(Function vsMain, int groupOrder)
        {
            ...
            if (reflectionMapType == TextureType.TwoD)
            {
                funcInvocation = new FunctionInvocation(FFPRenderState.FFPFunGenerateTexcoordEnvSphere, groupOrder, internalCounter++);
            ...
            {
                funcInvocation = new FunctionInvocation(FFPRenderState.FFPFuncGenerateTexCoordEnvReflect, groupOrder, internalCounter++);
        ...
        private bool AddPsInvocations(Function psMain, int groupOrder)
        {
            ...
            funcInvocation.PushOperand(psOutDiffuse, Operand.OpSemantic.In, (int)(Operand.OpMask.X | Operand.OpMask.Y | Operand.OpMask.Z));
            funcInvocation.PushOperand(reflectionPower, Operand.OpSemantic.In);
            funcInvocation.PushOperand(psOutDiffuse, Operand.OpSemantic.Out, (int)(Operand.OpMask.X | Operand.OpMask.Y | Operand.OpMask.Z));
        ...
        {
            get { return reflectionPowerValue; }
            set 
            { 
        ...
        {
            get { return reflectionMapTextureName; }
            set { reflectionMapTextureName = value; }
        }
        public string MaskMapTextureName
        {
            get { return maskMapTextureName; }
            set { maskMapTextureName = value; }
        }
        ...
        {
            get { return (int)FFPRenderState.FFPShaderStage.Texturing + 1; }
        }
    }

    class ReflectionMapFactory : SubRenderStateFactory
    {
        ...
        public override void WriteInstance(Serialization.MaterialSerializer ser, SubRenderState subRenderState, Pass srcPass, Pass dstPass)
        {
            //TODO
