...
{
    class GLSLESProgramWriter : ProgramWriter
    {
        Dictionary<Axiom.Graphics.GpuProgramParameters.GpuConstantType, string> gpuConstTypeMap;
        Dictionary<Parameter.SemanticType, string> paramSemanticMap;
        Dictionary<string, string> inputToGLStatesMap;
        Dictionary<FunctionInvocation, string> functionCacheMap;
        Dictionary<string, string> definesMap;
        Dictionary<Parameter.ContentType, string> contentToPerVertexAttributes;
        int glslVersion;
        List<string> fragInputParams;
        Dictionary<string, string> cachedFunctionLibraries;
        ...
        internal override void WriteSourceCode(System.IO.StreamWriter stream, Program program)
        {
            ...
            foreach (var curFunction in functionList)
            {
                ...
                {
                    FunctionInvocation funcInvoc = (FunctionInvocation)itAtom;
                    int itOperand = 0;
                    int itOperandEnd = funcInvoc.OperandList.Count;

                    StringBuilder localOs = new StringBuilder();
                        ...
                        if (opSemantic == Operand.OpSemantic.Out || opSemantic == Operand.OpSemantic.InOut)
                        {
                            ...
                            if (gpuType == GpuProgramType.Fragment)
                            {
                                if (fragInputParams.Contains(paramName))
                                {
                                    ...
                                    stream.WriteLine("\t" + gpuConstTypeMap[op.Parameter.Type] + " " + newVar + " = " + tempVar + ";\n");
                                ...
                                foreach (var param in parameterList)
                                {
                                    if (CompareUniformByName(param, paramName))
                                    {
                                        ...
                                        if (inputToGLStatesMap.ContainsKey(newVar) == false)
                                        {
                                            ...
                                            stream.WriteLine("\t" + gpuConstTypeMap[param.Type] + " " + newVar + " = " + paramName + ";\n");
        ...
        private void CacheDependencyFunctions(string libName)
        {
            ...
            Dictionary<string, string> functionCache = new Dictionary<string, string>();
            string line;
        ...
        private FunctionInvocation CreateInvocationFromString(string input)
        {
            ...
            for (int i = 0; i < parameters.Length; i++)
            {
                ...
                if (paramTokens.Length == 3)
                {
                    ...
                    Parameter p = new Parameter(gpuType, paramTokens[2], Parameter.SemanticType.Unknown, i, Parameter.ContentType.Unknown, 0);
        ...
        private void WriteProgramDependencies(StreamWriter stream, Program program)
        {
            ...
            List<FunctionInvocation> forwardDecl = new List<FunctionInvocation>();
            var functionList = program.Functions;
            int itFunction = 0;
            ...
            for (; itAtom != itAtomEnd; itAtom++)
            {
                ...
                FunctionInvocation funcInvoc = atomInstances[itAtom] as FunctionInvocation;
            ...
            {
                FunctionInvocation invoc = new FunctionInvocation(string.Empty, 0, 0, string.Empty);
        ...
        private void DiscoverFunctionDependencies(FunctionInvocation invoc, List<FunctionInvocation> depVector)
        {
            ...
            {
                Axiom.Core.LogManager.Instance.DefaultLog.Write("ERROR: Cached function not found " + invoc.FunctionName);
        ...
        static bool CompareUniformByName(UniformParameter param, string str)
        {
