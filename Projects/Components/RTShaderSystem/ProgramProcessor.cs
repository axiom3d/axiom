...
{
    /// <summary>
    /// Provides extra processing services on CPU based programs.
    /// The base classs only the generic processing. IN order to provide target langauge specific services and
    /// optimization one should dervie from this class and register its factory via the ProgramManger instance.
    /// </summary>
    public abstract class ProgramProcessor : IDisposable
    {
        ...
        static List<MergeCombination> paramMergeCombinations;
        Dictionary<Function, string> functionMap;
        ...
        /// <summary>
        /// Called before the creation of the GPU programs.
        /// Does several preperation operations such as validation, register compaction and specific target langauge optimizations.  
        /// </summary>
        ...
        internal static bool CompactVsOutputs(Function vsMain, Function fsMain)
        {
            ...
            if (ShaderGenerator.Instance.VertexShaderOutputsCompactPolicy == ShaderGenerator.OutputsCompactPolicy.Low && outTexCoordSlots <= maxTexCoordSlots)
                return true;
            ...
            List<MergeParameter> vsMergedParamsList = new List<MergeParameter>();
            List<MergeParameter> fsMergedParamsList = new List<MergeParameter>();
            List<Parameter> vsSplitParams = new List<Parameter>();
            List<Parameter> fsSplitParams = new List<Parameter>();
            ...
            Dictionary<Parameter, Parameter> vsLocalParamsMap = new Dictionary<Parameter, Parameter>();
            Dictionary<Parameter, Parameter> fsLocalParamsMap = new Dictionary<Parameter, Parameter>();

            GenerateLocalSplitParameters(vsMain, GpuProgramType.Vertex, vsMergedParamsList, vsSplitParams, vsLocalParamsMap);
            GenerateLocalSplitParameters(fsMain, GpuProgramType.Fragment, fsMergedParamsList, fsSplitParams, fsLocalParamsMap);
        ...
        internal static void GenerateLocalSplitParameters(Function func, GpuProgramType progType, List<MergeParameter> mergedParams, List<Parameter> splitParams, Dictionary<Parameter, Parameter> localParamsMap)
        {
            ...
            for (int i = 0; i < splitParams.Count; i++)
            {
                ...
                Parameter localParameter = func.ResolveLocalParameter(srcParameter.Semantic, srcParameter.Index, "lssplit_" + srcParameter.Name, srcParameter.Type);
            ...
            for (int i = 0; i < mergedParams.Count; i++)
            {
                ...
                for (int p = 0; p < curMergeParameter.SourceParameterCount; p++)
                {
                    ...
                    if (localParamsMap.ContainsKey(srcMergedParameter))
                    {
                        ...
                        {
                            FunctionInvocation curFuncInvocation = new FunctionInvocation(FFPRenderState.FFPFuncAssign, (int)FFPRenderState.FFPVertexShaderStage.VSPostProcess, invocationCounter++);

                            curFuncInvocation.PushOperand(localParamsMap[srcMergedParameter], Operand.OpSemantic.In, curMergeParameter.GetSourceParameterMask(p));
                            curFuncInvocation.PushOperand(curMergeParameter.GetDestinationParameter((int)Operand.OpSemantic.Out, i), Operand.OpSemantic.Out, curMergeParameter.GetDestinationParameterMask(p));
                            func.AddAtomInstance(curFuncInvocation);
                        }
                        else if (progType == GpuProgramType.Fragment)
                        {
                            FunctionInvocation curFuncInvocation = new FunctionInvocation(FFPRenderState.FFPFuncAssign, (int)FFPRenderState.FFPFragmentShaderStage.PSPreProcess, invocationCounter++);

                            curFuncInvocation.PushOperand(curMergeParameter.GetDestinationParameter((int)Operand.OpSemantic.In, i), Operand.OpSemantic.In, curMergeParameter.GetDestinationParameterMask(p));
                            curFuncInvocation.PushOperand(localParamsMap[srcMergedParameter], Operand.OpSemantic.Out, curMergeParameter.GetSourceParameterMask(p));
                            func.AddAtomInstance(curFuncInvocation);
                        }
                    }
        ...
        /// Rebuild the given parameters list using the merged parameters.
        /// </summary>
        internal static void RebuildFunctionInvocations(List<FunctionAtom> funcAtomList, List<MergeParameter> mergedParams, Dictionary<Parameter,Parameter> localParamsMap)
        {
            Dictionary<Parameter, List<Operand>> paramsRefMap = new Dictionary<Parameter, List<Operand>>();
        ...
        internal static void CountVsTexcoordOutputs(Function vsMain, out int outTexCoordSlots, out int outTexCoordFloats)
        {
            outTexCoordFloats = 0;
            outTexCoordSlots = 0;
        ...
        /// <param name="splitParams"></param>
        internal static void MergeParameters(List<Parameter>[] paramsTable, List<MergeParameter> mergedParams, List<Parameter> splitParams)
        {
        ...
        internal static void MergeParametersByPredefinedCombinations(List<Parameter>[] paramsTable, List<MergeParameter> mergedParams)
        {
            if (paramMergeCombinations == null)
            {
            ...
            if (ShaderGenerator.Instance.VertexShaderOutputsCompactPolicy == ShaderGenerator.OutputsCompactPolicy.Low ||
                ShaderGenerator.Instance.VertexShaderOutputsCompactPolicy == ShaderGenerator.OutputsCompactPolicy.Medium)
            {
                ...
                MergeCombination[] simpleCombination = new MergeCombination[6]
                {
        ...
        /// <returns></returns>
        internal static bool MergeParametersByCombination(MergeCombination combination, List<Parameter>[] paramsTable, out MergeParameter mergedParameter)
        {
            ...
            for (int i = 0; i < 4; ++i)
            {
                ...
                {
                    mergedParameter.AddSourceParameter(curParamList[curParamList.Count - 1], combination.srcParameterMask[srcParameterCount]);
        ...
        internal static void MergeParametersReminders(List<Parameter>[] paramsTable, List<MergeParameter> mergedParams, List<Parameter> splitParams)
        {
            ...
            int remindersFloatCount = (1 * paramsTable[0].Count) + (2 * paramsTable[1].Count) + (3 * paramsTable[2].Count) + (4 * paramsTable[3].Count);
            ...
            for (int slot = 0; slot < remindersTotalSlotCount; ++slot)
            {
                ...
                for (int row = 0; row < 4; ++row)
                {
                    ...
                    if (curParamList.Count > 0)
                    {
                        ...
                        curMergeParam.AddSourceParameter(curParamList[curParamList.Count - 1], (int)Operand.OpMask.All);
                        curParamList.RemoveAt(curParamList.Count - 1);
                        mergedParams.Add(curMergeParam);
                    }
                }
        ...
        internal static void BuildParameterReferenceMap(List<FunctionAtom> funcAtomList, Dictionary<Parameter, List<Operand>> paramsRefMap)
        {
            for (int i = 0; i < funcAtomList.Count; i++)
            {
                ...
                {
                    FunctionInvocation curFuncInvocation = curAtom as FunctionInvocation;
        ...
        /// <param name="mergedParams"></param>
        /// <param name="paramsRefMap"></param>
        internal static void ReplaceParametersReferences(List<MergeParameter> mergedParams, Dictionary<Parameter, List<Operand>> paramsRefMap)
        {
            for (int i = 0; i < mergedParams.Count; i++)
            {
                MergeParameter curMergeParameter = mergedParams[i];
        ...
        /// <param name="mergedParams"></param>
        /// <param name="paramsRefMap"></param>
        internal static void ReplaceSplitParametersReferences(Dictionary<Parameter, Parameter> localParamsMap, Dictionary<Parameter, List<Operand>> paramsRefMap)
        {
        ...
        /// <example>X|Y for FLOAT2 etc..</example>
        /// </summary>
        /// <param name="type"></param>
        /// <returns></returns>
        protected static int GetParameterMaskByType(Axiom.Graphics.GpuProgramParameters.GpuConstantType type)
        {
            ...
            switch (type)
            {
                ...
                case GpuProgramParameters.GpuConstantType.Float3:
                    ...
                    break;
                case GpuProgramParameters.GpuConstantType.Float4:
                    paramMask = ((int)Operand.OpMask.X | (int)Operand.OpMask.Y | (int)Operand.OpMask.Z | (int)Operand.OpMask.W);
                    break;
                default:
        ...
        {
           public int[] srcParameterTypeCount; //The count of each source type. I.E. (1 FLOAT1, 0 FLOAT2, 1 FLOAT3, 0 FLOAT4).
           public int[] srcParameterMask; // source parameters mask. OPM_ALL means all fields used, otherwise it is split source parameter.
