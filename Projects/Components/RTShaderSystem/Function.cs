...
namespace Axiom.Components.RTShaderSystem
{
    internal class Function : IDisposable
    {
        ...
        List<Parameter> inputParameters, outputParameters, localParameters;
        List<FunctionAtom> atomInstances;
        FunctionType functionType;
        AtomInstanceCompare _comparer = new AtomInstanceCompare();
        #endregion
        ...
        internal void AddOutputParameter(Parameter parameter)
        {
            ...
            {
                throw new AxiomException("Parameter <" + parameter.Name + "> has equal semantic parameter in function <" + Name + ">");
            }
        ...
        internal Parameter ResolveLocalParameter(Parameter.SemanticType semantic, int index, string name, Graphics.GpuProgramParameters.GpuConstantType type)
        {
        ...
        /// <param name="type">The type of the desired parameter</param>
        /// <returns>paramter instance in case of that resolve operation succeed</returns>
        internal Parameter ResolveLocalParamter(Parameter.SemanticType semantic, int index, Parameter.ContentType content, Graphics.GpuProgramParameters.GpuConstantType type)
        {
        ...
        internal Parameter ResolveInputParameter(Parameter.SemanticType semantic, int index, Parameter.ContentType content, Graphics.GpuProgramParameters.GpuConstantType type)
        {
            ...
            else
            {
                ...
                if (param != null & param.Content == content)
                {
                    ...
                    {
                        throw new AxiomException("Cannot resolve parameter - semantic: " + semantic.ToString() + " - index: " + index.ToString() + " due to type mimatch. Function <" + Name + ">");
        ...
        internal Parameter ResolveOutputParameter(Parameter.SemanticType semantic, int index, Parameter.ContentType content, Graphics.GpuProgramParameters.GpuConstantType type)
        {
            ...
            switch (semantic)
            {
                ...
                case Parameter.SemanticType.BlendWeights:
                case Parameter.SemanticType.BlendIndicies:
                    throw new AxiomException("Can not resolve parameter - semantic: " + semantic.ToString() + " - index: " + index.ToString() + " since support init is not implemented yet. Function <" + Name + ">");
                case Parameter.SemanticType.Normal:
        ...
        public static Parameter GetParameterBySemantic(List<Parameter> paramList, Parameter.SemanticType semantic, int index)
        {
            for (int it = 0; it < paramList.Count; it++)
            {
        ...
        /// <returns>null if no matching paramter found</returns>
        public static Parameter GetParameterByContent(List<Parameter> paramList, Parameter.ContentType content, Graphics.GpuProgramParameters.GpuConstantType type)
        {
    ...
    class AtomInstanceCompare : IComparer<FunctionAtom>
    {
