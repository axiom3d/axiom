...
namespace Axiom.Components.RTShaderSystem
{
    public class ShaderGenerator
    {
        ...
        List<SGPass> SGPassList = new List<SGPass>();
        Dictionary<SGTechnique, SGTechnique> SGTechniqueMap;
        Tuple<MatGroupPair, SGMaterial> SGMaterialMap;
        Dictionary<string, SGScheme> SGSchemeMap;
        Dictionary<string, SGScriptTranslator> SGScriptTranslatorMap;

        private static ShaderGenerator instance;
        SceneManager activeSceneMgr;
        Dictionary<string, SceneManager> sceneManagerMap;
        SgScriptTranslatorManager scriptTranslatorManager;
        Dictionary<string, SGScriptTranslator> scriptTranslatorMap;
        SGScriptTranslator coreScriptTranslator;
        string shaderLanguage;
        string vertexShaderProfiles;
        string[] vertexShaderProfilesList;
        string fragmentShaderProfiles;
        string[] fragmentShaderProfilesList;
        string shaderCachePath;
        ProgramManager programManager;
        ProgramWriterManager programWriteManager;
        FFPRenderStateBuilder ffpRenderStateBuilder;
        List<Tuple<MatGroupPair, SGMaterial>> materialEntriesMap;
        Dictionary<string, SGScheme> schemeEntriesMap;
        //List<Tuple<SGTechnique, SGTechnique>> techniqueEntriesMap;
        Dictionary<SGTechnique, SGTechnique> techniqueEntriesMap;
        Dictionary<string, SubRenderStateFactory> subRenderStateFactories;
        Dictionary<string, SubRenderStateFactory> subRenderStateExFactories;
        bool activeViewportValid;
        int[] lightCount = new int[3];
        OutputsCompactPolicy vsOutputCompactPolicy;
        bool createShaderOverProgrammablePass;
        bool isFinalizing;
        string GeneratedShadersGroupName = "ShaderGeneratorResourceGroup";
        #endregion
        ...
        public ShaderGenerator()
        {
          ShaderGenerator.DefaultSchemeName = "ShaderGeneratorDefaultScheme";
            ...
            lightCount[1] = 0;
            lightCount[2] = 0;
            vsOutputCompactPolicy = OutputsCompactPolicy.Low;
         ...
         */
	        ...
	        FragmentShaderProfiles = ("ps_4_0 ps_3_x ps_3_0 fp40 fp30 fp20 arbfp1 ps_2_x ps_2_a ps_2_b ps_2_0 ps_1_4 ps_1_3 ps_1_2 ps_1_1");
        }
        #endregion
        ...
        /// destroy any CPU/GPU program that was creted by this shader geneartor.
        /// </summary>
        ...
        /// <summary>
        /// Returns a global render state associated with the given scheme name.
        /// Modifying this render state will affect all techniques that belongs to that scheme.
        /// This is the best way to apply global chagnes to all techniques.
        /// After altering the render state one should call invalidateScheme method in order to regerenarte shaders.
        /// </summary>
        ...
        /// <summary>
        /// Gets render state of specific pass.
        ...
        /// <summary>
        /// Add sub render state factory. Plugins or 3rd party applications may implement sub classes of
        ...
        {
            return HasShaderBasedTechnique(materialName, ResourceGroupManager.AutoDetectResourceGroupName, srcTechniqueSchemeName, dstTechniqueSchemeName);
        }
        /// <summary>
        ...
        public bool HasShaderBasedTechnique(string materialName, string groupName, string srcTechnqiueSchemeName, string dstTechniqueSchemeName)
        {
        ...
        /// <returns>True on success. Failure may occur if the source technique is not FFP pure, 
        /// or different source technique is mapped to the requested destination scheme.
        /// </returns>
        ...
        {
            Material srcMat = (Material)MaterialManager.Instance.GetByName(materialName);
            if(srcMat == null)
                return false;
            ...
            foreach (var itMatEntry in materialEntriesMap)
	        {
               ...
               if(itMatEntry == materialEntriesMap[materialEntriesMap.Count - 1])
               {
                ...
                materialEntriesMap.Add(new Tuple<MatGroupPair, SGMaterial>(new MatGroupPair(materialName, trueGroupName), matEntry));
               }
               else
               {
            ...
            SGTechnique sgTechEntry = new SGTechnique(matEntry, srcTechnique, dstTechniqueSchemeName);
        ...
        {
            Material mat = (Material)MaterialManager.Instance.GetByName(materialName);
        ...
        {
            return RemoveShaderBasedTechnique(materialName, ResourceGroupManager.AutoDetectResourceGroupName, srcTechniqueSchemeName, dstTechniqueSchemeName);
        }
        /// <summary>
        ...
        public bool RemoveAllShaderBasedTechniques(string materialName, string groupName)
        {
            ...
            foreach (var itMatEntry in materialEntriesMap)
	        {
                ...
                while(matTechniqueEntries.Count > 0)
                {
	                ...
	                {
                        RemoveShaderBasedTechnique(materialName, itMatEntry.Item1.Item2, itTechEntry.SourceTechnique.SchemeName, 
        ...
        /// <summary>
        /// Invalidate a given scheme. This action will lead to shader regeneration of all techniques 
        /// that belong to the given scheme name.
        /// </summary>
        ...
        /// <summary>
        /// Validate a given scheme. This action will generate shader programs for all techniques 
        /// of the given scheme name.
        /// </summary>
        ...
        /// <summary>
        /// Invalidates specific material scheme. This action will lead to shader regeneration of the technique beongs to the
        /// given scheme name
        /// </summary>
        /// <param name="schemeName">The scheme to invalidate</param>
        ...
        /// <summary>
        /// Validate specific material scheme. This action will generate shader programs for the 
        /// technique of the given scheme name.
        /// </summary>
        /// <param name="schemeName">The scheme to validate</param>
        ...
        {
            Material srcMat = (Material)MaterialManager.Instance.GetByName(srcMaterialName);
            Material dstMat = (Material)MaterialManager.Instance.GetByName(dstMaterialName);
            ...
            List<int> schemesToRemove = new List<int>();
            ...
            foreach (var itSrcMatEntry in materialEntriesMap)
	        {
                ...
                foreach (var itTechEntry in techniqueEntries)
	            {
                    ...
                    if(CreateShaderBasedTechnique(dstMaterialName, trueDstGroupName, srcFromTechniqueScheme, srcToTechniqueScheme, false))
                    {
                        ...
                        for (short pi = 0; pi < passCount; pi++)
			            {
                            if(itTechEntry.HasRenderState(pi))
                            {
                                ...
                                RenderState dstRenderState = GetRenderState(srcToTechniqueScheme, dstMaterialName,trueDstGroupName, (ushort)pi);
        ...
        protected void notifyRenderSingleObject(IRenderable rend, Pass pass, AutoParamDataSource source, LightList lightList, bool suppressRenderStateChanges)
        {
        ...
        /// <returns></returns>
        public SubRenderState createSubRenderState(ScriptCompiler compiler, PropertyAbstractNode prop, TextureUnitState texState, SGScriptTranslator translator)
        {
        ...
        /// <param name="translator">The translator for the specific SubRenderState</param>
        /// <returns></returns>
        public SubRenderState createSubRenderState(ScriptCompiler compiler, PropertyAbstractNode prop, Pass pass, SGScriptTranslator translator)
        {
            SubRenderState subRenderState = null;
            foreach (var key in subRenderStateFactories.Keys)
            {
        ...
        internal SGScriptTranslator GetTranslator(AbstractNode node)
        {
            ...
            {
                ObjectAbstractNode obj = node as ObjectAbstractNode;
        ...
        {
            get{return scriptTranslatorMap.Count;}
        }
        /// <summary>
        ...
        internal void serializeTextureUnitStateAttributes(MaterialSerializer ser, SGPass passEntry, TextureUnitState srcTextureUnit)
        {
           //TODO
        }
        ...
        {
            get {return programManager.VertexShaderCount;}
        }
        /// <summary>
        ...
        {
            get {return programManager.FragmentShaderCount;}
        }
        ...
        {
            get{return subRenderStateFactories.Count;}
        }
        /// <summary>
        /// Gets/Sets the vertex shader outputs compaction policy
        /// </summary>
        public OutputsCompactPolicy VertexShaderOutputsCompactPolicy
        {
            get {return vsOutputCompactPolicy;}
            set {vsOutputCompactPolicy = value;}
        }
        /// <summary>
        /// Gets/Sets whether shaders are created for passes with shaders.
        ...
        /// </summary>
        public bool CreateShaderOverProgrammablePass
        {
            get{return createShaderOverProgrammablePass;}
            set {createShaderOverProgrammablePass = value;}
        }
        public int RTShaderSchemeCount
        {
            get {return schemeEntriesMap.Count;}
        }
        /// <summary>
        /// Gets/Set output shader cache path. Generated shader code will be written to this path.
            ...
            set
            {
                ...
                if(shaderCachePath != stdCachePath)
                {
                    ...
                    if(shaderCachePath != string.Empty)
                    {
                        ...
                        ResourceGroupManager.Instance.AddResourceLocation(shaderCachePath, "FileSystem", GeneratedShadersGroupName);
        ...
        {
            get{return vertexShaderProfilesList;}
        }
        /// <summary>
        ...
        {
             get{return fragmentShaderProfilesList;}
        }
        /// <summary>
        /// Gets/Sets the target shader langauge
        /// <remarks>The default shader language is cg.</remarks>
        /// </summary>
        public string TargetLangauge
        {
            get{return shaderLanguage;}
            set
            {
        ...
        /// <summary>
        /// Gets the active scenen manager that is doing the actual scene rendering.
        /// This attribute will be updated on the call to preFindVisibleObjects.
        /// </summary>
        ...
        {
            get{return ShaderGenerator.instance;}
        }
        #endregion
        ...
        internal class SGPass : IDisposable
        {
            ...
            /// <param name="lightList"></param>
            /// <param name="suppressRenderStateChanges"></param>
            public void NotifyRenderSingleObject(IRenderable rend, AutoParamDataSource source, LightList lightList, bool suppressRenderStateChanges)
            {
            ...
            {
                get{return customRenderState;}
                set{customRenderState = value;}
            }
        ...
        internal class SGTechnique : IDisposable
        {
            ...
            List<SGPass> passEntries;
            List<RenderState> customRenderStates;
            bool buildDstTechnique;
            public static string UserKey;
            ...
            {
                get{return _dstTechniqueSchemeName;}
            }
            /// <summary>
            /// Gets/Sets if the technique needs to generate shader code.
            /// </summary>
            public bool BuildDestinationTechnique
            {
                get{return this.buildDstTechnique;}
                set{this.buildDstTechnique = value;}
            }
            /// <summary>
        ...
        internal class SGScheme : IDisposable
        {
            ...
            public bool Empty
            {
               get{return (techniqueEntries.Count == 0);}
            }
            /// <summary>
        ...
        private void SynchronizeWithLightSettings()
        {
            ...
            if(sceneManager != null && curRenderState.LightCountAutoUpdate)
            {
                ...
                int[] sceneLightCount = new int[3]{0,0,0};
                int[] currLightCount = new int[3]{0,0,0};
            ...
            public RenderState  GetRenderState(string materialName, string groupName, ushort passIndex)
            {
                ...
                for (int i = 0; i < techniqueEntries.Count; i++)
			    {
                    ...
                    if((curMat.Name == materialName) && ((doAutoDetect == true) || (curMat.Group == groupName)))
                    {
        ...
        public class SgScriptTranslatorManager : ScriptTranslatorManager
        {
            ...
            {
                get { return _owner.NumTranslators; }
            }
        ...
        protected class MatGroupPair : Tuple<string, string>
        {
            string materialName;
