...
{
    class HardwareSkinningFactory : SubRenderStateFactory
    {
        static int HsMaxWeightCount = 4;
        int maxCalculableBoneCount;

        //A set of custom shadow caster materials
        Material[] customShadowCasterMaterialsLinear = new Material[HardwareSkinningFactory.HsMaxWeightCount];
        Material[] customShadowCasterMaerialsDualQuaternion = new Material[HardwareSkinningFactory.HsMaxWeightCount];

        //A set of custom shadow receiver materials
        Material[] customShadowReceiverMaterialsLinear = new Material[HardwareSkinningFactory.HsMaxWeightCount];
        Material[] customShadowReceiverMaterialsDualQuaternion = new Material[HardwareSkinningFactory.HsMaxWeightCount];
        ...
        public override SubRenderState CreateInstance(Scripting.Compiler.ScriptCompiler compiler, Scripting.Compiler.AST.PropertyAbstractNode prop, Pass pass, SGScriptTranslator stranslator)
        {
            if (prop.Name == "hardware_skinning")
            {
                ...
                else
                {
                    ...
                    HardwareSkinning hardSkinSrs = (HardwareSkinning)subRenderState;
        ...
        public void SetCustomShadowCasterMaterials(SkinningType skinningType, Material caster1Weight, Material caster2Weight, Material caster3Weight, Material caster4Weight)
        {
        ...
        public void SetCustomShadowReceiverMaterials(SkinningType skinningType, Material receiver1Weight, Material receiver2Weight, Material receiver3Weight, Material receiver4Weight)
        {
            if (skinningType == SkinningType.DualQuaternion)
            {
        ...
        {
            get { return HardwareSkinning.SGXType; }
        }
        public int MaxCalculableBoneCount
        {
            get { return maxCalculableBoneCount; }
            set { maxCalculableBoneCount = value; }
        }
        ...
        public void PrepareEntityForSkinning(Entity entity, SkinningType skinningType, bool correctAntipodalityHandling, bool shearScale)
        {
            if (entity != null)
            {
                //TODO!
                ...
                for (int indexLod = 0; indexLod < lodLevels; indexLod++)
                {
                    ...
                    for (int indexSub = 0; indexSub < numSubEntites; indexSub++)
                    {
                        ...
                        ImprintSkeletonData(mat, isValid, boneCount, weightCount, skinningType, correctAntipodalityHandling, shearScale);
        ...
        protected bool ExtractSkeletonData(Entity entity, out int boneCount, out int weightCount)
        {
            ...
            if (!hasVertexAnim && entity.HasSkeleton)
            {
                ...
                for (int i = 0; i < totalMeshes; i++)
                {
                    ...
                    VertexElement declWeights = ro.vertexData.vertexDeclaration.FindElementBySemantic(VertexElementSemantic.BlendWeights, 0);
                    VertexElement declIndexes = ro.vertexData.vertexDeclaration.FindElementBySemantic(VertexElementSemantic.BlendIndices, 0);
        ...
        protected bool ImprintSkeletonData(Material material, bool isValid, int boneCount, int weightCount, SkinningType skinningType, bool correctAntipodalityHandling, bool scalingShearingSupport)
        {
        ...
        {
            HardwareSkinning skin = new HardwareSkinning();
