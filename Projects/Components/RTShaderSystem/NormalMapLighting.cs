...
namespace Axiom.Components.RTShaderSystem
{
    ...
    public class NormalMapLighting : SubRenderState
    {
        ...
        static string SGXLibNormalMapLighting = "SGXLib_NormalMapLighting";
        static string SGXFuncConstructTbnMatrix = "SGX_ConstructTBNMatrix";
        static string SGXFuncTranformNormal = "SGX_TransformNormal";
        static string SGXFuncTransformPosition = "SGX_TransformPosition";
        static string SGXFuncFetchNormal = "SGX_FetchNormal";
        static string SGXFuncLightDirectionalDiffuse = "SGX_Light_Directional_Diffuse";
        static string SGXFuncLightDirectionDiffuseSpecular = "SGX_Light_Directional_DiffuseSpecular";
        ...
        string normalMapTextureName;
        TrackVertexColor trackVertexColorType;
        bool specularEnabled;
        List<LightParams> lightParamsList = new List<LightParams>();
        int normalMapSamplerIndex;
        int vsTexCoordSetIndex;
        FilterOptions normalMapMinFilter;
        FilterOptions normalMapMagFilter;
        FilterOptions normalMapMipfilter;
        uint normalMapAnisotropy;
        Real normalMapMipBias;
        NormalMapSpace normalMapSpace;
        UniformParameter worldMatrix, worldInvRotMatrix, camPosWorldSpace;
        Parameter vsInPosition, vsWorldPosition, vsOutView;
        Parameter psInView, vsInNormal, vsInTangent;
        Parameter vsTBNMatrix;
        Parameter vsLocalDir;
        UniformParameter normalMapSampler;
        Parameter psNormal, vsInTexcoord, vsOutTexcoord, psInTexcoord;
        Parameter psTempDiffuseColor, psTempSpecularColor, psDiffuse, psSpecular, psOutDiffuse, psOutSpecular;
        UniformParameter derivedSceneColor, lightAmbientColor, derivedAmbientLightColor;
        UniformParameter surfaceAmbientColor, surfaceDiffuseColor, surfaceSpecularColor, surfaceEmissiveColor;
        UniformParameter surfaceShininess;
        static Light blankLight; //Shared blank light 
        #endregion
        ...
        public void GetNormalMapFiltering(out FilterOptions minFilter, out FilterOptions magFilter, out FilterOptions mipFilter)
        {
        ...
        public override void UpdateGpuProgramsParams(IRenderable rend, Pass pass, AutoParamDataSource source, Core.Collections.LightList lightList)
        {
            ...
            Vector3 vRow0 = new Vector3(matWorld[0, 0], matWorld[0, 1], matWorld[0, 2]);
            Vector3 vRow1 = new Vector3(matWorld[1, 0], matWorld[1, 1], matWorld[1, 2]);
            Vector3 vRow2 = new Vector3(matWorld[2, 0], matWorld[2, 1], matWorld[2, 2]);
        ...
        private bool ResolvePerLightParameters(ProgramSet programSet)
        {
            ...
            for (int i = 0; i < lightParamsList.Count; i++)
            {
                ...
                {
                    case LightType.Directional:
                        lightParamsList[i].Direction = vsProgram.ResolveParameter(GpuProgramParameters.GpuConstantType.Float4, -1, GpuProgramParameters.GpuParamVariability.PerObject, "light_direction_obj_space");
                        ...
                        if (normalMapSpace == RTShaderSystem.NormalMapSpace.Tangent)
                        {
                            ...
                            lightParamsList[i].VSOutDirection = vsMain.ResolveOutputParameter(Parameter.SemanticType.TextureCoordinates, -1, pctToUse, GpuProgramParameters.GpuConstantType.Float3);
                        }
                        else if (normalMapSpace == RTShaderSystem.NormalMapSpace.Object)
                        {
                            ...
                            lightParamsList[i].VSOutToLightDir = vsMain.ResolveOutputParameter(Parameter.SemanticType.TextureCoordinates, -1, pctToUse, GpuProgramParameters.GpuConstantType.Float3);

                        }
                        if (lightParamsList[i].VSOutToLightDir == null)
                            return false;

                        lightParamsList[i].PSInDirection = psMain.ResolveInputParameter(Parameter.SemanticType.TextureCoordinates, lightParamsList[i].VSOutToLightDir.Index, lightParamsList[i].VSOutToLightDir.Content, lightParamsList[i].VSOutToLightDir.Type);
                    ...
                    case LightType.Point:
                        ...
                        lightParamsList[i].Position = vsProgram.ResolveParameter(GpuProgramParameters.GpuConstantType.Float4, -1, GpuProgramParameters.GpuParamVariability.Lights | GpuProgramParameters.GpuParamVariability.PerObject, "light_position_world_space");
                        if (lightParamsList[i].Position == null)
                            return false;

                        if (normalMapSpace == RTShaderSystem.NormalMapSpace.Tangent)
                        {
                            lightParamsList[i].VSOutToLightDir = vsMain.ResolveOutputParameter(Parameter.SemanticType.TextureCoordinates, -1, this.IntToEnum(RTShaderSystem.NormalMapSpace.Tangent, i), GpuProgramParameters.GpuConstantType.Float3);
                        }
                        else if (normalMapSpace == RTShaderSystem.NormalMapSpace.Object)
                        {
                            lightParamsList[i].VSOutToLightDir = vsMain.ResolveOutputParameter(Parameter.SemanticType.TextureCoordinates, -1, this.IntToEnum(RTShaderSystem.NormalMapSpace.Object, i), GpuProgramParameters.GpuConstantType.Float3);
                        }
                        if (lightParamsList[i].VSOutToLightDir == null)
                            return false;

                        lightParamsList[i].PSInToLightDir = psMain.ResolveInputParameter(Parameter.SemanticType.TextureCoordinates,
                            lightParamsList[i].VSOutToLightDir.Index, lightParamsList[i].VSOutToLightDir.Content, lightParamsList[i].VSOutToLightDir.Type);
                        ...
                        if (vsLocalDir == null)
                        {
                            vsLocalDir = vsMain.ResolveLocalParameter(Parameter.SemanticType.Unknown, 0, "lNormalMapTempDir", GpuProgramParameters.GpuConstantType.Float3);
                            if (vsLocalDir == null)
                                return false;
                        }
                        ...
                        {
                            vsWorldPosition = vsMain.ResolveLocalParameter(Parameter.SemanticType.Position, 0, Parameter.ContentType.PositionWorldSpace, GpuProgramParameters.GpuConstantType.Float3);
                            if (vsWorldPosition == null)
                                return false;
                        }
                        ...
                        if (worldInvRotMatrix == null)
                        {
                            worldInvRotMatrix = vsProgram.ResolveParameter(GpuProgramParameters.GpuConstantType.Matrix_4X4, -1, GpuProgramParameters.GpuParamVariability.PerObject, "inv_world_rotation_matrix");
                            if (worldInvRotMatrix == null)
                                return false;
                        }
                        break;
                    case LightType.Spotlight:
                        ...
                        lightParamsList[i].PSInToLightDir = psMain.ResolveInputParameter(Parameter.SemanticType.TextureCoordinates, lightParamsList[i].VSOutToLightDir.Index, lightParamsList[i].VSOutToLightDir.Content, lightParamsList[i].VSOutToLightDir.Type);
                        if (lightParamsList[i].PSInToLightDir == null)
                            return false;

                        lightParamsList[i].Direction = vsProgram.ResolveParameter(GpuProgramParameters.GpuConstantType.Float4, -1, GpuProgramParameters.GpuParamVariability.Lights | GpuProgramParameters.GpuParamVariability.PerObject, "light_direction_obj_space");
                        if (lightParamsList[i].Direction == null)
                            return false;
                        if (normalMapSpace == RTShaderSystem.NormalMapSpace.Tangent)
                        {
                            lightParamsList[i].VSOutDirection = vsMain.ResolveOutputParameter(Parameter.SemanticType.TextureCoordinates, -1, this.IntToEnum(RTShaderSystem.NormalMapSpace.Tangent, i), GpuProgramParameters.GpuConstantType.Float3);
                            if (lightParamsList[i].VSOutDirection == null)
                                return false;
                        }
                        else if (normalMapSpace == RTShaderSystem.NormalMapSpace.Object)
                        {
                            lightParamsList[i].VSOutDirection = vsMain.ResolveOutputParameter(Parameter.SemanticType.TextureCoordinates, -1, this.IntToEnum(RTShaderSystem.NormalMapSpace.Object, i), GpuProgramParameters.GpuConstantType.Float3);
                            if (lightParamsList[i].VSOutDirection == null)
                                return false;
                        }
                        if (lightParamsList[i].VSOutDirection == null)
                        ...
                        lightParamsList[i].PSInDirection = psMain.ResolveInputParameter(Parameter.SemanticType.TextureCoordinates, lightParamsList[i].VSOutDirection.Index, lightParamsList[i].VSOutDirection.Content, lightParamsList[i].VSOutDirection.Type);
                        if (lightParamsList[i].PSInDirection == null)
                            return false;
                ...
                if ((trackVertexColorType & TrackVertexColor.Diffuse) == 0)
                {
                    lightParamsList[i].DiffuseColor = psProgram.ResolveParameter(GpuProgramParameters.GpuConstantType.Float4, -1, GpuProgramParameters.GpuParamVariability.Lights, "derived_light_diffuse");
                ...
                if (specularEnabled)
                {
                    ...
                    {
                        lightParamsList[i].SpecularColor = psProgram.ResolveParameter(GpuProgramParameters.GpuConstantType.Float4, -1, GpuProgramParameters.GpuParamVariability.Lights, "derived_light_specular");
        ...
        private bool ResolveGlobalParameters(ProgramSet programSet)
        {
            ...
            normalMapSampler = psProgram.ResolveParameter(Axiom.Graphics.GpuProgramParameters.GpuConstantType.Sampler2D, normalMapSamplerIndex, GpuProgramParameters.GpuParamVariability.PerObject, "gNormalMapSampler");
            ...
            else
            {
                ...
                surfaceAmbientColor = psProgram.ResolveAutoParameterInt(GpuProgramParameters.AutoConstantType.SurfaceAmbientColor, 0, 0);
            ...
            if ((trackVertexColorType & TrackVertexColor.Specular) == 0)
            {
                surfaceSpecularColor = psProgram.ResolveAutoParameterInt(GpuProgramParameters.AutoConstantType.SurfaceSpecularColor, 0);
            ...
            vsInNormal = vsMain.ResolveInputParameter(Parameter.SemanticType.Normal, 0, Parameter.ContentType.NormalObjectSpace, Axiom.Graphics.GpuProgramParameters.GpuConstantType.Float3);
            if (vsInNormal == null)
                return false;
            ...
            if (normalMapSpace == RTShaderSystem.NormalMapSpace.Tangent)
            {
                vsInTangent = vsMain.ResolveInputParameter(Parameter.SemanticType.Tangent, 0, Parameter.ContentType.TangentObjectSpace, GpuProgramParameters.GpuConstantType.Float3);
                if (vsInTangent == null)
                    return false;

                //Resolve local vertex shader TNB matrix
                vsTBNMatrix = vsMain.ResolveLocalParameter(Parameter.SemanticType.Unknown, 0, "lMatTBN", GpuProgramParameters.GpuConstantType.Matrix_3X3);
                if (vsTBNMatrix == null)
                    return false;
            }
            ...
            vsInTexcoord = vsMain.ResolveInputParameter(Parameter.SemanticType.TextureCoordinates, vsTexCoordSetIndex, texCoordToUse, GpuProgramParameters.GpuConstantType.Float2);
            if (vsInTexcoord == null)
                return false;
            
            //Resolve output vertex shader texture coordinates
            vsOutTexcoord = vsMain.ResolveOutputParameter(Parameter.SemanticType.TextureCoordinates, -1, texCoordToUse, GpuProgramParameters.GpuConstantType.Float2);
            if (vsOutTexcoord == null)
                return false;

            //resolve pixel input texture coordinates normal
            psInTexcoord = psMain.ResolveInputParameter(Parameter.SemanticType.TextureCoordinates, vsOutTexcoord.Index, vsOutTexcoord.Content, vsOutTexcoord.Type);
            ...
            if (normalMapSpace == RTShaderSystem.NormalMapSpace.Object)
            {
                psNormal = psMain.ResolveLocalParameter(Parameter.SemanticType.Normal, 0, Parameter.ContentType.NormalObjectSpace, GpuProgramParameters.GpuConstantType.Float3);
                if (psNormal == null)
                    return false;
            }
            else if (normalMapSpace == RTShaderSystem.NormalMapSpace.Tangent)
            {
                psNormal = psMain.ResolveLocalParameter(Parameter.SemanticType.Normal, 0, Parameter.ContentType.NormalTangentSpace, GpuProgramParameters.GpuConstantType.Float3);
                if (psNormal == null)
                    return false;
            }
            ...
            psTempDiffuseColor = psMain.ResolveLocalParameter(Parameter.SemanticType.Unknown, 0, "lNormalMapDiffuse", GpuProgramParameters.GpuConstantType.Float4);
            ...
            if (specularEnabled)
            {
                ...
                psTempSpecularColor = psMain.ResolveLocalParameter(Parameter.SemanticType.Unknown, 0, "lNormalMapSpecular", GpuProgramParameters.GpuConstantType.Float4);
                ...
                if (normalMapSpace == RTShaderSystem.NormalMapSpace.Tangent)
                {
                    vsOutView = vsMain.ResolveOutputParameter(Parameter.SemanticType.TextureCoordinates, -1, Parameter.ContentType.PostOCameraTangentSpace, GpuProgramParameters.GpuConstantType.Float3);
                    if (vsOutView == null)
                        return false;
                }
                else if (normalMapSpace == RTShaderSystem.NormalMapSpace.Object)
                ...
                psInView = psMain.ResolveInputParameter(Parameter.SemanticType.TextureCoordinates, vsOutView.Index, vsOutView.Content, vsOutView.Type);
                if (psInView == null)
                    return false;

                //Resolve camera position world space
                camPosWorldSpace = vsProgram.ResolveAutoParameterInt(GpuProgramParameters.AutoConstantType.CameraPosition, 0);
        ...
        internal override bool AddFunctionInvocations(ProgramSet programSet)
        {
            ...
            if (AddVSInvocation(vsMain, (int)FFPRenderState.FFPVertexShaderStage.VSLighting, internalCounter) == false)
                return false;
        ...
        protected bool AddVSInvocation(Function vsMain, int groupOrder, int internalCounter)
        {
            ...
            if (vsInPosition != null && vsOutView != null)
            {
                ...
                curFuncInvocation.PushOperand(camPosWorldSpace, Operand.OpSemantic.In, ((int)Operand.OpMask.X | (int)Operand.OpMask.Y | (int)Operand.OpMask.Z));
                curFuncInvocation.PushOperand(vsWorldPosition, Operand.OpSemantic.In, ((int)Operand.OpMask.X | (int)Operand.OpMask.Y | (int)Operand.OpMask.Z));
            ...
            for (int i = 0; i < lightParamsList.Count; i++)
            {
                if(AddVSIlluminationInvocation(lightParamsList[i], vsMain, groupOrder, ref internalCounter) == false)
                    return false;
            }
            return true;
        }
        internal bool AddVSIlluminationInvocation(LightParams curLightParams, Function vsMain, int groupOrder, ref int internalCounter)
        {
            FunctionInvocation curFuncInvocation = null;
            ...
            if (curLightParams.Direction != null && curLightParams.VSOutDirection != null)
            {
                ...
                if (normalMapSpace == RTShaderSystem.NormalMapSpace.Tangent)
                {
                    curFuncInvocation = new FunctionInvocation(SGXFuncTranformNormal, groupOrder, internalCounter++);
                    curFuncInvocation.PushOperand(vsTBNMatrix, Operand.OpSemantic.In);
                    curFuncInvocation.PushOperand(curLightParams.Direction, Operand.OpSemantic.In, ((int)Operand.OpMask.X | (int)Operand.OpMask.Y | (int)Operand.OpMask.Z));
            ...
            if (curLightParams.Position != null && curLightParams.VSOutToLightDir != null)
            {
                ...
                curFuncInvocation = new FunctionInvocation(FFPRenderState.FFPFuncSubtract, groupOrder, internalCounter++);
                curFuncInvocation.PushOperand(curLightParams.Position, Operand.OpSemantic.In, ((int)Operand.OpMask.X | (int)Operand.OpMask.Y | (int)Operand.OpMask.Z));
        ...
        protected bool AddPSGlobalIlluminationInvocation(Function psMain, int groupOrder,ref int internalCount)
        {
            ...
            else
            {
                if ((trackVertexColorType & TrackVertexColor.Ambient) != 0)
                {
                    curFuncInvocation = new FunctionInvocation(FFPRenderState.FFPFuncModulate, groupOrder, internalCount++);
                ...
                {
                    curFuncInvocation = new FunctionInvocation(FFPRenderState.FFPFuncAssign, groupOrder, internalCount++);
                    curFuncInvocation.PushOperand(derivedAmbientLightColor, Operand.OpSemantic.In, ((int)Operand.OpMask.X | (int)Operand.OpMask.Y | (int)Operand.OpMask.Z));
                    curFuncInvocation.PushOperand(psDiffuse, Operand.OpSemantic.In, ((int)Operand.OpMask.X | (int)Operand.OpMask.Y | (int)Operand.OpMask.Z));
                    curFuncInvocation.PushOperand(psTempDiffuseColor, Operand.OpSemantic.Out);
                    psMain.AddAtomInstance(curFuncInvocation);
                }
        ...
        protected bool AddPSIlluminationInvocation(LightParams curLightParams, Function psMain, int groupOrder, ref int internalCounter)
        {
            FunctionInvocation curFuncInvocation = null;
            ...
            if ((trackVertexColorType & TrackVertexColor.Diffuse) != 0)
            {
                ...
                curFuncInvocation.PushOperand(psDiffuse, Operand.OpSemantic.In, ((int)Operand.OpMask.X | (int)Operand.OpMask.Y | (int)Operand.OpMask.Z));
                curFuncInvocation.PushOperand(curLightParams.DiffuseColor, Operand.OpSemantic.In, ((int)Operand.OpMask.X | (int)Operand.OpMask.Y | (int)Operand.OpMask.Z));
                curFuncInvocation.PushOperand(curLightParams.DiffuseColor, Operand.OpSemantic.Out, ((int)Operand.OpMask.X | (int)Operand.OpMask.Y | (int)Operand.OpMask.Z));
                psMain.AddAtomInstance(curFuncInvocation);
            }
            ...
            if(specularEnabled && (trackVertexColorType & TrackVertexColor.Specular) != 0)
            {
                curFuncInvocation = new FunctionInvocation(FFPRenderState.FFPFuncModulate, groupOrder, internalCounter++);
                curFuncInvocation.PushOperand(psDiffuse, Operand.OpSemantic.In, ((int)Operand.OpMask.X | (int)Operand.OpMask.Y | (int)Operand.OpMask.Z));
                curFuncInvocation.PushOperand(curLightParams.SpecularColor, Operand.OpSemantic.In, ((int)Operand.OpMask.X | (int)Operand.OpMask.Y | (int)Operand.OpMask.Z));
                curFuncInvocation.PushOperand(curLightParams.SpecularColor, Operand.OpSemantic.Out, ((int)Operand.OpMask.X | (int)Operand.OpMask.Y | (int)Operand.OpMask.Z));
                psMain.AddAtomInstance(curFuncInvocation);
            }
            ...
            switch (curLightParams.Type)
	        {
		        case LightType.Directional:
                    if(specularEnabled)
                    {
                        curFuncInvocation = new FunctionInvocation(SGXFuncLightDirectionDiffuseSpecular, groupOrder, internalCounter++);
                        curFuncInvocation.PushOperand(psNormal, Operand.OpSemantic.In, ((int)Operand.OpMask.X | (int)Operand.OpMask.Y | (int)Operand.OpMask.Z));
                        curFuncInvocation.PushOperand(psInView, Operand.OpSemantic.In, ((int)Operand.OpMask.X | (int)Operand.OpMask.Y | (int)Operand.OpMask.Z));
                        curFuncInvocation.PushOperand(curLightParams.PSInDirection, Operand.OpSemantic.In, ((int)Operand.OpMask.X | (int)Operand.OpMask.Y | (int)Operand.OpMask.Z));
                        curFuncInvocation.PushOperand(curLightParams.DiffuseColor, Operand.OpSemantic.In, ((int)Operand.OpMask.X | (int)Operand.OpMask.Y | (int)Operand.OpMask.Z));
                        curFuncInvocation.PushOperand(curLightParams.SpecularColor, Operand.OpSemantic.In, ((int)Operand.OpMask.X | (int)Operand.OpMask.Y | (int)Operand.OpMask.Z));
                        curFuncInvocation.PushOperand(surfaceShininess, Operand.OpSemantic.In);
                        curFuncInvocation.PushOperand(psTempDiffuseColor, Operand.OpSemantic.In, ((int)Operand.OpMask.X | (int)Operand.OpMask.Y | (int)Operand.OpMask.Z));
                        curFuncInvocation.PushOperand(psTempSpecularColor, Operand.OpSemantic.In, ((int)Operand.OpMask.X | (int)Operand.OpMask.Y | (int)Operand.OpMask.Z));
                        curFuncInvocation.PushOperand(psTempDiffuseColor, Operand.OpSemantic.Out, ((int)Operand.OpMask.X | (int)Operand.OpMask.Y | (int)Operand.OpMask.Z));
                        curFuncInvocation.PushOperand(psTempSpecularColor, Operand.OpSemantic.Out, ((int)Operand.OpMask.X | (int)Operand.OpMask.Y | (int)Operand.OpMask.Z));
                        psMain.AddAtomInstance(curFuncInvocation);
                    }
                    else
                    {
        ...
        case LightType.Point:
                    if(specularEnabled)
                    {
                        ...
                        curFuncInvocation.PushOperand(curLightParams.PSInToLightDir, Operand.OpSemantic.In, ((int)Operand.OpMask.X | (int)Operand.OpMask.Y | (int)Operand.OpMask.Z));
        ...
        {
            get { return vsTexCoordSetIndex; }
            set { vsTexCoordSetIndex = value; }
        }
        public NormalMapSpace NormalMapSpace
        {
            get { return normalMapSpace; }
            set { normalMapSpace = value; }
        }
        public string NormalMapTextureName
        {
            get { return normalMapTextureName; }
            set { normalMapTextureName = value; }
        }
        public uint NormalMapAnisotropy
        {
            get { return normalMapAnisotropy; }
            set { normalMapAnisotropy = value; }
        }
        public Real NormalMapMipBias
        {
            get { return normalMapMipBias; }
        }
