	...
	public class PCZTestApp : TechDemo
	{
		...
		public override void CreateScene()
		{
			...
			*/
			...
			raySceneQuery = scene.CreateRayQuery( new Ray( camera.ParentNode.Position, Vector3.NegativeUnitZ ) );
		...
		private PCZone createTerrainZone( string zoneName, string terrain_cfg )
		{
			...
			p = ( (PCZSceneManager)scene ).CreatePortal( portalName, PORTAL_TYPE.PORTAL_TYPE_AABB );
			...
			Portal p2;
			p2 = ( (PCZSceneManager)scene ).CreatePortal( portalName, PORTAL_TYPE.PORTAL_TYPE_AABB );
			corners[ 0 ] = aabb.Minimum;
			corners[ 1 ] = aabb.Maximum;
			...
			p.setTargetZone( ( (PCZSceneManager)scene ).DefaultZone );
