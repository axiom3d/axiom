...
namespace Axiom.Demos
{
	...
	public class CubeMapping : TechDemo
	{
		...
		private void PrepareEntity( string meshName )
		{
			...
			{
				originalMesh = (Mesh)MeshManager.Instance.Load( meshName, ResourceGroupManager.DefaultResourceGroupName, BufferUsage.StaticWriteOnly, BufferUsage.StaticWriteOnly, true, true, 1 );
			...
			unsafe void UpdateNoise()
		{
			...
			for ( int i = 0; i < clonedMesh.SubMeshCount; i++ )
			{
				...
				if ( subMesh.useSharedVertices )
				{
					...
					UpdateVertexDataNoiseAndNormals( clonedMesh.SharedVertexData, originalMesh.SharedVertexData, subMesh.indexData, sharedNormals );
				}
				else
				{
...
#endif
		{
			...
			{
				float n = 1 + displacement * Noise3( orgPosPtr[ i ] / density + tm, orgPosPtr[ i + 1 ] / density + tm, orgPosPtr[ i + 2 ] / density + tm );
		...
		private void ToggleBlending()
		{
			...
			currentLbx = (LayerBlendOperationEx)Enum.Parse( typeof ( LayerBlendOperationEx ), blendModes[ currentLbxIndex ], true );

			PrepareEntity( meshes[ currentMeshIndex ] );

			// update the UI
			OverlayManager.Instance.Elements.GetElement( "Example/CubeMapping/Material" ).Text = string.Format( "[M] Material: {0}", blendModes[ currentLbxIndex ] );
		}
		...
		private void ToggleNoise()
		{
			...
			OverlayManager.Instance.Elements.GetElement( "Example/CubeMapping/Noise" ).Text = string.Format( "[N] Noise: {0}", noiseOn ? "on" : "off" );
		}
		...
		public override bool Setup()
		{
			if ( base.Setup() )
			{
				...
				ToggleMesh();
				ToggleBlending();
				OverlayManager.Instance.Elements.GetElement( "Example/CubeMapping/CubeMap" ).Text = string.Format( "[C] CubeMap: {0}", cubeMaps[ currentCubeIndex ] );
				return true;
			}
