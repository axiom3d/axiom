...
namespace Axiom.SceneManagers.PagingLandscape.Data2D
{
    ...
    /// </summary>
    public class Data2D_HeightFieldN : Data2D
    {
        #region Fields
        ...
        private float Scale
        {
            get
            {
                ...
                // Acording to ogre plugin it should be /256 but i get no height at all...
                // return Options.Instance.Scale.y / 256;
            }
        }
        ...
        protected override void load(float X, float Z)
        {
            if (image == null)
            {
                image = Image.FromFile(Options.Instance.Landscape_Filename + ".HN." + Z.ToString() + "." + X.ToString() + "." + Options.Instance.Landscape_Extension);
                ...
                {
                    float h = image.Data[i + (bpp - 1)] * this.Scale;
