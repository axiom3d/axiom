...
namespace Axiom.Graphics
{
    ...
    public partial class GpuProgramParameters : DisposableObject
    {
        ...
        {
            _writeRawConstants( physicalIndex, new float[] { val }, 1 );
        }

        [OgreVersion( 1, 7, 2790 )]
        public void WriteRawConstant( int physicalIndex, int val )
        {
            _writeRawConstants( physicalIndex, new[] { val }, 1 );
        }
        ...
        public void CopyMatchingNamedConstantsFrom( GpuProgramParameters source )
        {
            ...
            foreach ( var i in source._namedConstants.Map )
            {
                ...
                {
                    Memory.Copy(
                        source.GetFloatPointer( olddef.PhysicalIndex ).Pointer,
                        GetFloatPointer( newdef.PhysicalIndex ).Pointer,
                        sz * sizeof( float )
                        );
                }
                else
                {
                    Memory.Copy(
                        source.GetIntPointer( olddef.PhysicalIndex ).Pointer,
