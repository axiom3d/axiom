            ...
            for ( int z = dst.Front; z < dst.Back; z++, sz_48 += stepz )
            {
                ...
                for ( int y = dst.Top; y < dst.Bottom; y++, sy_48 += stepy )
                {
                    ...
                    for ( int x = dst.Left; x < dst.Right; x++, sx_48 += stepx )
                    {
                        ...
                        ColorEx accum = x1y1z1 * ( ( 1.0f - sxf ) * ( 1.0f - syf ) * ( 1.0f - szf ) ) + x2y1z1 * ( sxf * ( 1.0f - syf ) * ( 1.0f - szf ) ) + x1y2z1 * ( ( 1.0f - sxf ) * syf * ( 1.0f - szf ) ) + x2y2z1 * ( sxf * syf * ( 1.0f - szf ) ) + x1y1z2 * ( ( 1.0f - sxf ) * ( 1.0f - syf ) * szf ) + x2y1z2 * ( sxf * ( 1.0f - syf ) * szf ) + x1y2z2 * ( ( 1.0f - sxf ) * syf * szf ) + x2y2z2 * ( sxf * syf * szf );
