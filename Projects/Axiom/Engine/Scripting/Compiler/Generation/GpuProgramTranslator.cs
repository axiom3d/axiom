...
namespace Axiom.Scripting.Compiler
{
	...
	{
		#region Nested type: GpuProgramTranslator
		...
		public class GpuProgramTranslator : Translator
		{
			...
			[OgreVersion( 1, 7, 2 )]
			public static void TranslateProgramParameters( ScriptCompiler compiler, GpuProgramParameters parameters, ObjectAbstractNode obj )
			{
									...
									if ( defFound )
									{
										switch ( def.DataType )
										{
											...
											case GpuProgramParameters.AutoConstantDataType.Int:
												...
												else
												{
													...
													if ( i2 == null )
													{
														...
														{
															compiler.AddError( CompileErrorCode.NumberExpected, prop.File, prop.Line, "extra parameters required by constant definition " + atom1.Value );
														}
													}
													else
													{
														...
														{
															compiler.AddError( CompileErrorCode.InvalidParameters, prop.File, prop.Line, "invalid auto constant extra info parameter" );
														}
													}
												}
												break;
											...
											case GpuProgramParameters.AutoConstantDataType.Real:
												if ( def.AutoConstantType == GpuProgramParameters.AutoConstantType.Time || def.AutoConstantType == GpuProgramParameters.AutoConstantType.FrameTime )
												{
													Real f = 1.0f;
													if ( i2 != null )
													{
												...
												else
												{
													if ( i2 != null )
													{
														...
														{
															compiler.AddError( CompileErrorCode.InvalidParameters, prop.File, prop.Line, "incorrect float argument definition in extra parameters" );
														}
													}
													else
													{
			...
			protected void _translateGpuProgram( ScriptCompiler compiler, ObjectAbstractNode obj )
			{
				...
				if ( !processed )
				{
					prog = GpuProgramManager.Instance.CreateProgram( obj.Name, compiler.ResourceGroup, source, _translateIDToGpuProgramType( obj.Id ), syntax );
				}
				else
				{
			...
			protected void _translateHighLevelGpuProgram( ScriptCompiler compiler, ObjectAbstractNode obj )
			{
						...
						else
						{
							...
							foreach ( AbstractNode it in prop.Values )
							{
								if ( it is AtomAbstractNode )
								{
									...
									if ( prop.Name == "attach" )
									{
										ScriptCompilerEvent evt = new ProcessResourceNameScriptCompilerEvent( ProcessResourceNameScriptCompilerEvent.ResourceType.GpuProgram, ( it ).Value );
				...
				if ( !processed )
				{
					prog = ( HighLevelGpuProgramManager.Instance.CreateProgram( obj.Name, compiler.ResourceGroup, language, _translateIDToGpuProgramType( obj.Id ) ) );
			...
			protected void _translateUnifiedGpuProgram( ScriptCompiler compiler, ObjectAbstractNode obj )
			{
				...
				if ( !processed )
				{
					prog = ( HighLevelGpuProgramManager.Instance.CreateProgram( obj.Name, compiler.ResourceGroup, "unified", _translateIDToGpuProgramType( obj.Id ) ) );
				}
				else
				{
					prog = (HighLevelGpuProgram)progObj;
