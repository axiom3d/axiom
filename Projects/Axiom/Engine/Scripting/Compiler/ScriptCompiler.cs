        ...
        private void _overlayObject( AbstractNode source, ObjectAbstractNode dest )
        {
            if ( source.Type == AbstractNodeType.Object )
            {
                ...
                foreach ( AbstractNode i in src.Children )
                {
                    ...
                    {
                        overrides.Add( new KeyValuePair<AbstractNode, AbstractNode>( i, dest.Children[ dest.Children.Count - 1 ] ) );
                    }
                    else
                    {
                ...
                for ( int i = 0; i < dest.Children.Count; i++ )
                {
                    if ( dest.Children[ i ].Type == AbstractNodeType.Object )
                    {
                        ...
                        bool nodeHasWildcard = node.Name.Contains( "*" );
                    ...
                    if ( i.Type == AbstractNodeType.Object )
                    {
                        ...
                        if ( !overridden[ node ] )
                        {
                            ...
                            if ( overrideIndex < overrides.Count )
                            {
                                ...
                                {
                                    ObjectAbstractNode temp = (ObjectAbstractNode)overrides[ j ].Key;
                                    if ( string.IsNullOrEmpty( temp.Name ) && temp.Cls == node.Cls && overrides[ j ].Value == dest.Children[ dest.Children.Count - 1 ] )
                                    {
                ...
                {
                    if ( overrides[ i ].Value != dest.Children[ dest.Children.Count - 1 ] )
                    {
