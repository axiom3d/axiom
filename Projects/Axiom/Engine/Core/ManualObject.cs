...
namespace Axiom.Core
{
    ...
    public class ManualObject : MovableObject
    {
        ...
        protected virtual void CopyTempVertexToBuffer()
        {
            ...
            int sourceIndex = this.declSize * ( rop.vertexData.vertexCount - 1 );
            byte[] pBase = new byte[ this.tempVertexBuffer.Length - sourceIndex ];
            Array.Copy( this.tempVertexBuffer, sourceIndex, pBase, 0, pBase.Length );

            foreach ( VertexElement elem in elemList )
            {
                float[] pFloat = new float[ 3 ];
                uint pRGBA;
                ...
                switch ( elem.Type )
                {
                    ...
                    case VertexElementType.Float3:
                        for ( int i = 0; i < VertexElement.GetTypeCount( elem.Type ); i++ )
                        {
                            pFloat[ i ] = BitConverter.ToSingle( pBase, elem.Offset + sizeof( float ) * i );
                        }
                        break;
                    ...
                    case VertexElementType.Color_ARGB:
                        pRGBA = BitConverter.ToUInt32( pBase, elem.Offset );
                        break;
                ...
                switch ( elem.Semantic )
                {
                    case VertexElementSemantic.Position:
                        pFloat[ 0 ] = this.tempVertex.position.x;
                        pFloat[ 1 ] = this.tempVertex.position.y;
                        pFloat[ 2 ] = this.tempVertex.position.z;
                        break;

                    case VertexElementSemantic.Normal:
                        pFloat[ 0 ] = this.tempVertex.normal.x;
                        pFloat[ 1 ] = this.tempVertex.normal.y;
                        pFloat[ 2 ] = this.tempVertex.normal.z;
                        break;
                    ...
                    case VertexElementSemantic.TexCoords:
                        ...
                        {
                            pFloat[ t ] = this.tempVertex.texCoord[ elem.Index ][ t ];
                        }
                        break;
