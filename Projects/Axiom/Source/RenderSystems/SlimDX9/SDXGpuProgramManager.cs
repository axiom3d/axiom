...
namespace Axiom.RenderSystems.SlimDX9
{
    ...
    /// </summary>
    public class SDXGpuProgramManager : GpuProgramManager
    {
        protected D3D.Device device;

        internal SDXGpuProgramManager( D3D.Device device )
        {
            this.device = device;
        }
        ...
        protected override Resource _create( string name, ResourceHandle handle, string group, bool isManual, IManualResourceLoader loader, GpuProgramType type, string syntaxCode )
        {
            ...
            {
                return new SDXVertexProgram( this, name, handle, group, isManual, loader, device );
            }
            else
            {
                return new SDXFragmentProgram( this, name, handle, group, isManual, loader, device );
            }
        }
