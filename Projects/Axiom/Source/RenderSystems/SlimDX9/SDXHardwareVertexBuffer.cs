...
{
    /// <summary>
    /// 	Summary description for SDXHardwareVertexBuffer.
    /// </summary>
    public class SDXHardwareVertexBuffer : HardwareVertexBuffer
    {
        ...
        private TimingMeter vbufferLockTimer = MeterManager.GetMeter( "Buffer Lock", "Axiom.RenderSystems.SlimDX" );

        #endregion

        #region Constructors

        public SDXHardwareVertexBuffer( int vertexSize, int numVertices, BufferUsage usage,
            ...
            : base( vertexSize, numVertices, usage, useSystemMemory, useShadowBuffer )
        {
            ...
            d3dBuffer = new D3D.VertexBuffer( device,
                                              sizeInBytes,
                                              SDXHelper.ConvertEnum( usage ),
                                              D3D.VertexFormat.None,
                                              d3dPool );
        }

        ~SDXHardwareVertexBuffer()
        {
            if ( d3dBuffer != null )
            {
                d3dBuffer.Dispose();
        ...
        {
            D3D.LockFlags d3dLocking = SDXHelper.ConvertEnum( locking, usage );

            SlimDX.DataStream s = d3dBuffer.Lock( offset, length, d3dLocking );
