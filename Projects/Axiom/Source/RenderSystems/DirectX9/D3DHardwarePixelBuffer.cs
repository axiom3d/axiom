...
namespace Axiom.RenderSystems.DirectX9
{
	...
	public class D3DHardwarePixelBuffer : HardwarePixelBuffer
	{
		...
		///</summary>
		protected static void FromD3DLock( PixelBox rval, int pitch, DX.GraphicsStream stream )
		{
		...
		///</summary>
		protected static void FromD3DLock( PixelBox rval, D3D.LockedBox lbox, DX.GraphicsStream stream )
		{
		...
		public override void BlitToMemory( BasicBox srcBox, PixelBox dst )
		{
			...
			if ( surface != null )
			{
				...
				D3D.SurfaceLoader.FromSurface( subSurface, destRect, surface, srcRect, D3D.Filter.None, 0 );

				// Lock temp surface and copy it to memory
				int pitch; // Filled in by D3D
				DX.GraphicsStream data = subSurface.LockRectangle( D3D.LockFlags.ReadOnly, out pitch );
				// Copy it
			...
			else
			{
				...
				D3D.VolumeLoader.FromVolume( subVolume, ddestBox, volume, dsrcBox, D3D.Filter.None, 0 );
				// Lock temp surface and copy it to memory
				D3D.LockedBox lbox; // Filled in by D3D
				DX.GraphicsStream data = subVolume.LockBox( D3D.LockFlags.ReadOnly, out lbox );
				// Copy it
				PixelBox locked = new PixelBox( dst.Width, dst.Height, dst.Depth, tmpFormat );
		...
		public void GenMipmaps()
		{
			...
			{
				// Software mipmaps
				D3D.TextureLoader.FilterTexture( mipTex, 0, D3D.Filter.Box );
			}
		}
