...
namespace Axiom.RenderSystems.Xna
{
	...
	public class XnaRenderSystem : RenderSystem
	{
		...
		public static readonly Matrix4 ProjectionClipSpace2DToImageSpaceOrtho = new Matrix4(
			...
			0, 0, 0, 1f,
			0, 0, 0, 1f );
		XFG.GraphicsDevice device;
		XFG.GraphicsDeviceCapabilities d3dCaps;
		...
		protected void SetVertexBufferBinding( VertexBufferBinding binding )
		{
			...
			while ( e.MoveNext() )
			{
				...
				device.Vertices[ stream ].SetSource( buffer.D3DVertexBuffer, 0, buffer.VertexSize );
			...
			{
				device.Vertices[ i ].SetSource( null, 0, 0 );
			}
		...
		public override void Render( RenderOperation op )
		{
			...
			if ( op.useIndices )
			{
				...
				if ( device.VertexShader != null && device.PixelShader != null )
					device.DrawIndexedPrimitives( primType, op.vertexData.vertexStart, 0, op.vertexData.vertexCount, op.indexData.indexStart, primCount );
			...
			device.Vertices[ 2 ].SetSource( null, 0, 0 );
			device.Indices = null;
		...
		public override ColorEx AmbientLight
		{
			...
			set
			{
				ambientColorParameter.SetValue( new Microsoft.Xna.Framework.Vector4( value.r * 255, value.g * 255, value.b * 255, value.a * 255 ) );
				//              effect.AmbientLightColor = new Microsoft.Xna.Framework.Vector3(value.r * 255, value.g * 255, value.b * 255) ;
			}
		}
		...
		public override bool DepthCheck
		{
			...
			set
			{
				device.RenderState.DepthBufferEnable = value;
			}
		}
		...
		public override Axiom.Math.Matrix4 ProjectionMatrix
		{
			...
			set
			{
				XNA.Matrix mat = MakeD3DMatrix( value );
		...
		private XNA.Matrix MakeD3DMatrix( Axiom.Math.Matrix4 matrix )
		{
			XNA.Matrix dxMat = new Microsoft.Xna.Framework.Matrix();
			...
			set
			{
				switch ( value )
				{
					case SceneDetailLevel.Points:
						device.RenderState.FillMode = Microsoft.Xna.Framework.Graphics.FillMode.Point;
						break;
					case SceneDetailLevel.Wireframe:
						device.RenderState.FillMode = Microsoft.Xna.Framework.Graphics.FillMode.WireFrame;
						break;
					case SceneDetailLevel.Solid:
						device.RenderState.FillMode = Microsoft.Xna.Framework.Graphics.FillMode.Solid;
						break;
				}
			}
		}
		...
		public override Axiom.Math.Matrix4 ViewMatrix
		{
			...
			set
			{
				...
				XNA.Matrix dxView = MakeD3DMatrix( viewMatrix );
		...
		public override Axiom.Math.Matrix4 WorldMatrix
		{
			...
			{
				//throw new Exception("The method or operation is not implemented.");
				worldParameter.SetValue( MakeD3DMatrix( value ) );
		...
		public override void ClearFrameBuffer( FrameBuffer buffers, ColorEx color, float depth, int stencil )
		{
			...
			device.Clear( flags, col, depth, stencil );
		}
		...
		public override RenderWindow CreateRenderWindow( string name, int width, int height, int colorDepth, bool isFullscreen, int left, int top, bool depthBuffer, bool vsync, object target )
		{
			if ( device == null )
			{
				...
				effect = new XFG.Effect( device, effectCode, XFG.CompilerOptions.Debug, null );
		...
		private XFG.GraphicsDevice InitDevice( bool isFullscreen, bool depthBuffer, int width, int height, int colorDepth, Control target )
		{
			...
			try
			{
				...
				newDevice = new Microsoft.Xna.Framework.Graphics.GraphicsDevice
					(
		...
		{
			return new XnaHardwareOcclusionQuery( device );
		}
		...
		{
			//todo
			device.RenderState.AlphaTestEnable = ( func != Axiom.Graphics.CompareFunction.AlwaysPass );
		...
		{
			//todo
			Microsoft.Xna.Framework.Graphics.ColorWriteChannels val = 0;
			...
			device.RenderState.ColorWriteChannels = val;
		}
		...
		public override void SetFog( Axiom.Graphics.FogMode mode, ColorEx color, float density, float start, float end )
		{
			...
			{
				device.RenderState.FogTableMode = Microsoft.Xna.Framework.Graphics.FogMode.None;
			...
			else
			{
				...
				device.RenderState.FogVertexMode = XnaHelper.ConvertEnum( mode );
				device.RenderState.FogTableMode = XnaHelper.ConvertEnum( mode );// Microsoft.Xna.Framework.Graphics.FogMode.Linear;
				device.RenderState.FogColor = col;
				...
				/*  effect2.FogEnabled = true;
				  effect2.FogColor = new Microsoft.Xna.Framework.Vector3(col.R, col.G, col.B);// col;
				  effect2.FogEnd = end;
		...
		public override void SetScissorTest( bool enable, int left, int top, int right, int bottom )
		{
			if ( enable )
			{
				device.ScissorRectangle = new XNA.Rectangle( left, top, right - left, bottom - top );
		...
		public override void SetSurfaceParams( ColorEx ambient, ColorEx diffuse, ColorEx specular, ColorEx emissive, float shininess )
		{
			...
			//ambient.ToXnaColor();
			ambientColorParameter.SetValue( new Microsoft.Xna.Framework.Vector4( col.R, col.G, col.B, col.A ) );


			/*   effect2.AmbientLightColor =new Microsoft.Xna.Framework.Vector3(col.R,col.G,col.B);
			   col = diffuse.ToXnaColor();
			   effect2.DiffuseColor = new Microsoft.Xna.Framework.Vector3(col.R, col.G, col.B);
			   col = specular.ToXnaColor();
			   effect2.SpecularColor = new Microsoft.Xna.Framework.Vector3(col.R, col.G, col.B);
			   col = emissive.ToXnaColor();
			   effect2.EmissiveColor = new Microsoft.Xna.Framework.Vector3(col.R, col.G, col.B);
		...
		public override void SetTexture( int stage, bool enabled, string textureName )
		{
			...
			else
			{
				if ( texStageDesc[ stage ].tex != null )
				{
					...
					//device.Textures[2].state.TextureState[stage].ColorOperation = Microsoft.Xna.Framework.Graphics.TextureOperation.Disable;
				}
		...
		{
			Microsoft.Xna.Framework.Graphics.TextureAddressMode d3dMode = XnaHelper.ConvertEnum( texAddressingMode );
		...
		{
			Microsoft.Xna.Framework.Graphics.BlendFunction d3dTexOp = XnaHelper.ConvertEnum( blendMode.operation );

			// TODO: Verify byte ordering
			if ( blendMode.operation == LayerBlendOperationEx.BlendManual )
			{
				device.RenderState.BlendFactor = new Microsoft.Xna.Framework.Graphics.Color( 0, 0, 0,
			...
			if ( blendMode.blendType == LayerBlendType.Color )
			{
				...
				device.RenderState.BlendFunction = d3dTexOp;
			}
			else if ( blendMode.blendType == LayerBlendType.Alpha )
			{
				// Make call to set operation
				device.RenderState.AlphaBlendOperation = d3dTexOp;
				//  device.RenderState.AlphaFunction= d3dTexOp;
			}

			// Now set up sources
			System.Drawing.Color factor = System.Drawing.Color.FromArgb( device.RenderState.BlendFactor.A,
																			device.RenderState.BlendFactor.R,
																			device.RenderState.BlendFactor.G,
																			device.RenderState.BlendFactor.B );
		...
		{
			Microsoft.Xna.Framework.Matrix d3dMat = Microsoft.Xna.Framework.Matrix.Identity;
		   ...
		   * */
			...
			{
				if ( d3dCaps.VertexProcessingCapabilities.SupportsTextureGenerationSphereMap )
				{
			...
			if ( texStageDesc[ stage ].autoTexCoordType == TexCoordCalcMethod.EnvironmentMapReflection )
			{
				...
				Microsoft.Xna.Framework.Matrix viewMatrix = viewParameter.GetValueMatrix();// effect.View;// this.viewMatrix;// device.Transform.View;
		...
		public override void SetTextureUnitFiltering( int stage, FilterType type, Axiom.Graphics.FilterOptions filter )
		{
			...
			Microsoft.Xna.Framework.Graphics.TextureFilter texFilter = XnaHelper.ConvertEnum( type, filter, d3dCaps, texType );

			switch ( type )
			{
				case FilterType.Min:
					device.SamplerStates[ stage ].MinFilter = texFilter;
					break;

				case FilterType.Mag:
					device.SamplerStates[ stage ].MagFilter = texFilter;
					break;

				case FilterType.Mip:
					device.SamplerStates[ stage ].MipFilter = texFilter;
					break;
			}
		}
		...
		public override void SetViewport( Axiom.Core.Viewport viewport )
		{
			if ( activeViewport != viewport || viewport.IsUpdated )
			{
				...
				if ( oriDSB == null )
				{
					oriDSB = device.DepthStencilBuffer;
				}
				// get the back buffer surface for this viewport          
				Microsoft.Xna.Framework.Graphics.RenderTarget2D back = (Microsoft.Xna.Framework.Graphics.RenderTarget2D)activeRenderTarget.GetCustomAttribute( "D3DBACKBUFFER" );

				device.SetRenderTarget( 0, back );

				Microsoft.Xna.Framework.Graphics.DepthStencilBuffer depth = (Microsoft.Xna.Framework.Graphics.DepthStencilBuffer)activeRenderTarget.GetCustomAttribute( "D3DZBUFFER" );
				...
				if ( depth.IsDisposed )
					device.DepthStencilBuffer = oriDSB;// new DepthStencilBuffer(device, oriDSB.Width, oriDSB.Height, DepthFormat.Depth24Stencil8, MultiSampleType.None, 0);
				...
				Microsoft.Xna.Framework.Graphics.Viewport d3dvp = new Microsoft.Xna.Framework.Graphics.Viewport();
		...
		private void CheckCaps( XFG.GraphicsDevice device )
		{
			...
			if ( d3dCaps.DeclarationTypeCapabilities.SupportsByte4 )
			{
			...
			// Hardware mipmapping?
			if ( d3dCaps.DriverCapabilities.CanAutoGenerateMipMap )
			{
			...
			// Cube mapping?
			if ( d3dCaps.TextureCapabilities.SupportsCubeMap )
			{
			...
			// Scissor test
			if ( d3dCaps.RasterCapabilities.SupportsScissorTest )
			{
				caps.SetCap( Capabilities.ScissorTest );
			}

			// 2 sided stencil
			if ( d3dCaps.StencilCapabilities.SupportsTwoSided )
			{
				caps.SetCap( Capabilities.TwoSidedStencil );
			}

			// stencil wrap
			if ( d3dCaps.StencilCapabilities.SupportsIncrement && d3dCaps.StencilCapabilities.SupportsDecrement )
			{
			...
			switch ( vpMajor )
			{
				case 1:
					...
					// 4d float vectors
					caps.VertexProgramConstantFloatCount = d3dCaps.MaxVertexShaderConstants;
			...
			if ( caps.CheckCap( Capabilities.VertexPrograms ) )
			{
				...
				XFG.GraphicsAdapter details = XFG.GraphicsAdapter.Adapters[ driver.AdapterNumber ];
	...
	internal struct D3DTextureStageDesc
	{
		...
		/// texture 
		public Microsoft.Xna.Framework.Graphics.Texture tex;
	}
