	...
	public class SceneManager
	{
		...
		protected virtual Pass DeriveShadowReceiverPass( Pass pass )
		{
			if ( IsShadowTechniqueTextureBased )
			{
				...
				if ( IsShadowTechniqueAdditive )
				{
					...
					retPass.SetRunOncePerLight( pass.RunOncePerLight,
		...
		protected internal virtual void CreateShadowTextures( ushort size, ushort count, PixelFormat format )
		{
			...
			for ( ushort t = 0; t < count; ++t )
			{
				...
				Texture shadowTex = TextureManager.Instance.GetByName( targName );
				if ( shadowTex == null )
				{
					shadowTex =
						TextureManager.Instance.CreateManual( targName, TextureType.TwoD,
															 size, size, 0, format,
