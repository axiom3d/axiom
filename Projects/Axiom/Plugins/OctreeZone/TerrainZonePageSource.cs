...
public class TerrainZonePageSource
{
    ...
    public virtual TerrainZonePage BuildPage(Real[] heightData, Material pMaterial)
    {
        ...
        if (mTerrainZone.PCZSM.HasSceneNode(name))
        {
            page.PageSceneNode = mTerrainZone.PCZSM.GetSceneNode(name);
        ...
        for (int j = 0; j < mPageSize - 1; j += (mTileSize - 1))
        {
            ...
            for (int i = 0; i < mPageSize - 1; i += (mTileSize - 1))
            {
                ...
                if (mTerrainZone.PCZSM.HasSceneNode(name))
                {
                    c = mTerrainZone.PCZSM.GetSceneNode(name);
                ...
                // set queue
                tile.RenderQueueGroup = mTerrainZone.PCZSM.WorldGeometryRenderQueueId;
                // Initialise the tile
