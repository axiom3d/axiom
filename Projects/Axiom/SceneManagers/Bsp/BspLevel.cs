...
namespace Axiom.SceneManagers.Bsp
{
    ...
    public class BspLevel : Resource
    {
        ...
        protected void LoadQuake3Level( Quake3Level q3lvl )
        {
            ...
            HardwareVertexBuffer vbuf = HardwareBufferManager.Instance.CreateVertexBuffer( decl.Clone( 0 ), q3lvl.NumVertices + this.patchVertexCount, BufferUsage.StaticWriteOnly /* the vertices will be read often for texture lighting, use shadow buffer */, true );

            // Create the vertex buffer for texture lighting, allow space for patches
            HardwareVertexBuffer texLightBuf = HardwareBufferManager.Instance.CreateVertexBuffer( decl.Clone( 1 ), q3lvl.NumVertices + this.patchVertexCount, BufferUsage.DynamicWriteOnly, false );
            ...
            for ( int v = 0; v < q3lvl.NumVertices; v++ )
            {
                ...
                using ( BufferBase bvptr = BufferBase.Wrap( vert ) )
                {
                    using ( BufferBase tlptr = BufferBase.Wrap( texLightMap ) )
                    {
            ...
            while ( face-- > 0 )
            {
                ...
                else if ( src.type == BspFaceType.Patch )
                {
                    ...
                    else
                    {
                        ...
                        dest.patchSurf = this.patches[ face ];
                    }
                }
            ...
            for ( int i = 0; i < q3lvl.NumNodes; i++ )
            {
                ...
                // Set plane
                splitPlane.Normal = new Vector3( q3lvl.Planes[ q3node.plane ].normal[ 0 ], q3lvl.Planes[ q3node.plane ].normal[ 1 ], q3lvl.Planes[ q3node.plane ].normal[ 2 ] );
                splitPlane.D = -q3lvl.Planes[ q3node.plane ].distance;

                node.SplittingPlane = splitPlane;

                // Set bounding box
                node.BoundingBox = new AxisAlignedBox( new Vector3( q3node.bbox[ 0 ], q3node.bbox[ 1 ], q3node.bbox[ 2 ] ), new Vector3( q3node.bbox[ 3 ], q3node.bbox[ 4 ], q3node.bbox[ 5 ] ) );
            ...
            for ( int i = 0; i < q3lvl.NumBrushes; i++ )
            {
                ...
                while ( numBrushSides-- > 0 )
                {
                    ...
                    var brushSide = new Plane( new Vector3( q3lvl.Planes[ q3lvl.BrushSides[ brushSideIdx ].planeNum ].normal[ 0 ], q3lvl.Planes[ q3lvl.BrushSides[ brushSideIdx ].planeNum ].normal[ 1 ], q3lvl.Planes[ q3lvl.BrushSides[ brushSideIdx ].planeNum ].normal[ 2 ] ), q3lvl.Planes[ q3lvl.BrushSides[ brushSideIdx ].planeNum ].distance );
    ...
    public class ManagedBufferBspVertex : ManagedBuffer, ITypePointer<BspVertex>
    {
        ...
        #region ITypePointer<BspVertex> Members
