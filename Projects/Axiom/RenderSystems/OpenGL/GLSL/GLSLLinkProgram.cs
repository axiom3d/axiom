...
namespace Axiom.RenderSystems.OpenGL.GLSL
{
	...
	public class GLSLLinkProgram : IDisposable
	{
		...
		#region Nested type: UniformReference
		...
		#region Nested type: AttributeSet

		public class AttributeSet : HashSet<uint> { }

		#endregion

		#region Nested type: UniformReferenceList
		...
		public int GetGLGeometryInputPrimitiveType( OperationType operationType, bool requiresAdjacency )
		{
			switch ( operationType )
			{
				...
				case OperationType.LineStrip:
					return requiresAdjacency ? Gl.GL_LINES_ADJACENCY_EXT : Gl.GL_LINES;
				default:
				case OperationType.TriangleList:
				case OperationType.TriangleStrip:
				case OperationType.TriangleFan:
					return requiresAdjacency ? Gl.GL_TRIANGLES_ADJACENCY_EXT : Gl.GL_TRIANGLES;
			}
		}
		...
		private void BuildUniformReferences()
		{
			...
			{
				GpuProgramParameters.GpuConstantDefinitionMap vertParams = GpuProgramParameters.GpuConstantDefinitionMap.Empty;
				GpuProgramParameters.GpuConstantDefinitionMap fragParams = GpuProgramParameters.GpuConstantDefinitionMap.Empty;
				GpuProgramParameters.GpuConstantDefinitionMap geomParams = GpuProgramParameters.GpuConstantDefinitionMap.Empty;
				if ( this.vertexProgram != null )
				{
				...
				GLSLLinkProgramManager.Instance.ExtractUniforms( this.glHandle, vertParams, geomParams, fragParams, this.uniformReferences );
					...
					if ( ( def.Variability & mask ) != 0 )
					{
						...
						switch ( def.ConstantType )
						{
							case GpuProgramParameters.GpuConstantType.Float1:
								using ( GpuProgramParameters.OffsetArray<float>.FixedPointer ptr = parameters.GetFloatPointer( def.PhysicalIndex ) )
								{
							...
							case GpuProgramParameters.GpuConstantType.Int1:
								using ( GpuProgramParameters.OffsetArray<int>.FixedPointer ptr = parameters.GetIntPointer( def.PhysicalIndex ) )
								{
		...
		private void CompileAndLink()
		{
			...
			if ( this.geometryProgram != null )
			{
				...
				Gl.glProgramParameteriEXT( this.glHandle, Gl.GL_GEOMETRY_INPUT_TYPE_EXT, GetGLGeometryInputPrimitiveType( inputOperationType, this.geometryProgram.IsAdjacencyInfoRequired ) );
				...
				Gl.glProgramParameteriEXT( this.glHandle, Gl.GL_GEOMETRY_OUTPUT_TYPE_EXT, GetGLGeometryOutputPrimitiveType( outputOperationType ) );

				Gl.glProgramParameteriEXT( this.glHandle, Gl.GL_GEOMETRY_VERTICES_OUT_EXT, this.geometryProgram.GLSLProgram.MaxOutputVertices );
			}
