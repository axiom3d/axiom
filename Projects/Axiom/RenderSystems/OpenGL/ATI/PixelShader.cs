	...
	public class PixelShader : Compiler2Pass
	{
		...
		*/
		...
		private void TestCompile( string testName, string snippet, Symbol[] expectedResults, int[] machineInstResults )
		{
			...
			Console.WriteLine( "  Pass 1 Lines scanned: {0}, Tokens produced: {1} out of {2}: {3}", currentLine, tokenInstructions.Count, expectedResults.Length, ( tokenInstructions.Count == expectedResults.Length ) ? passed : failed );
			...
			if ( machineInstResults != null )
			{
				...
				Console.WriteLine( "  Pass 2 Machine Instructions generated: {0} out of {1}: {2}", MIcount, machineInstResults.Length, ( MIcount == machineInstResults.Length ) ? passed : failed );

				Console.WriteLine( "    Validating Pass 2:" );

				for ( int i = 0; i < MIcount; i++ )
				{
					Console.WriteLine( "    instruction[{0}] = {1} : {2} : {3}", i, GetMachineInst( i ), machineInstResults[ i ], ( GetMachineInst( i ) == machineInstResults[ i ] ) ? passed : failed );
				}
				...
				for ( int i = 0; i < 4; i++ )
				{
					Console.WriteLine( "    Constants[{0}] = {1} : {2}", i, constants[ i ], ( (float)constants[ i ] == ( 1.0f + i ) ) ? passed : failed );
				}
			}
