...
//     <id value="$Id: SDXTexture.cs 1413 2008-11-11 14:29:46Z borrillis $"/>
// </file>
...
{
    /// <summary>
    /// Summary description for SDXTexture.
    /// </summary>
    /// <remarks>
    ...
    /// </remarks>
    public sealed class SDXTexture : Texture
    {
        private TimingMeter _textureLoadMeter = MeterManager.GetMeter( "Texture Load", "SDXTexture" );
        ...
        ///     List of subsurfaces
        /// </summary>
        private List<SDXHardwarePixelBuffer> _surfaceList = new List<SDXHardwarePixelBuffer>();
        ...
        {
            get { return _volumeTexture; }
        }

        internal D3D.Direct3D _manager;

        public SDXTexture( ResourceManager parent, string name, ResourceHandle handle, string group, bool isManual, IManualResourceLoader loader, D3D.Device device, D3D.Direct3D manager )
        ...
        private void LoadNormalTexture()
        {
            ...
            using ( AutoTimer auto = new AutoTimer( _textureLoadMeter ) )
            {
                if ( Name.EndsWith( ".dds" ) )
                {
                    ...
                    SetSrcAttributes( desc.Width, desc.Height, 1, SDXHelper.ConvertEnum( desc.Format ) );
                    SetFinalAttributes( desc.Width, desc.Height, 1, SDXHelper.ConvertEnum( desc.Format ) );
        ...
        private void LoadVolumeTexture()
        {
            ...
            if ( Name.EndsWith( ".dds" ) )
            {
                ...
                SetSrcAttributes( desc.Width, desc.Height, desc.Depth, SDXHelper.ConvertEnum( desc.Format ) );
        ...
        private void CreateCubeTexture()
        {
            ...
            {
                //_texture.AutoGenerateFilterType = GetBestFilterMethod(); //CHECK
        ...
        private void CreateSurfaceList()
        {
            ...
            if ( !updateOldList )
            {
                ...
                for ( int face = 0; face < faceCount; ++face )
                {
                    ...
                    {
                        SDXHardwarePixelBuffer buffer = new SDXHardwarePixelBuffer( bufusage );
        ...
        {
            foreach ( SDXHardwarePixelBuffer buf in _surfaceList )
            {
                buf.Dispose();
            }
        ...
        private SDXHardwarePixelBuffer GetSurfaceAtLevel( int face, int mip )
        {
        ...
        public void CopyToTexture( Axiom.Core.Texture target )
        {
            ...
            SDXTexture texture = (SDXTexture)target;
        ...
        private D3D.Format ChooseD3DFormat()
        {
            ...
            return SDXHelper.ConvertEnum( SDXHelper.GetClosestSupported( Format ) );
        }
        ...
        private void SetFinalAttributes( int width, int height, int depth, PixelFormat format )
        {
            ...
            {
                LogManager.Instance.Write( "SlimDX : ***** Dimensions altered by the render system" );
                LogManager.Instance.Write( "SlimDX : ***** Source image dimensions : {0}x{1}", SrcWidth, SrcHeight );
                LogManager.Instance.Write( "SlimDX : ***** Texture dimensions :  {0}x{1}", Width, Height );
            }
