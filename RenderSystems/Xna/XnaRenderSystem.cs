...
namespace Axiom.RenderSystems.XNA
{
    ...
    public class XNARenderSystem : RenderSystem
    {
        ...
        /// </summary>
        protected D3D.Graphics.GraphicsDeviceCapabilities d3dCaps;
        /// <summary>
        ...
        {
            D3D.Graphics.ClearOptions flags = 0;

            if ( ( buffers & FrameBuffer.Color ) > 0 )
            {
                flags |= D3D.Graphics.ClearOptions.Target;
            }
            if ( ( buffers & FrameBuffer.Depth ) > 0 )
            {
                flags |= D3D.Graphics.ClearOptions.DepthBuffer;
            }
                ...
                && caps.CheckCap( Capabilities.StencilBuffer ) )
            {
                ...
                flags |= D3D.Graphics.ClearOptions.Stencil;
            }

            // clear the device using the specified params
            device.Clear( flags, DX.Graphics.Color.White, depth, stencil );
        }
        ...
        public override RenderWindow CreateRenderWindow( string name, int width, int height, int colorDepth, bool isFullscreen, int left, int top, bool depthBuffer, bool vsync, object target )
        {
            ...
            catch ( D3D.Graphics.OutOfVideoMemoryException e )
            {
        ...
        private D3D.Graphics.GraphicsDevice InitDevice( bool isFullscreen, bool depthBuffer, int width, int height, int colorDepth, Control target )
        {
            if ( device != null )
            {
                return device;
            ...
            //D3D.Graphics.GraphicsDevice.IsUsingEventHandlers = false;

            D3D.Graphics.GraphicsDevice newDevice;

            // create the D3D Device, trying for the best vertex support first, and settling for less if necessary
            D3D.Graphics.GraphicsAdapter adapter = D3D.Graphics.GraphicsAdapter.Adapters[ 0 ];
            D3D.Graphics.DeviceType type = D3D.Graphics.DeviceType.Hardware;
#if DEBUG
            for ( int i = 0; i < D3D.Graphics.GraphicsAdapter.Adapters.Count; i++ )
            {
                if ( D3D.Graphics.GraphicsAdapter.Adapters[ i ].Description == "NVIDIA NVPerfHUD" )
                {
                    adapter = D3D.Graphics.GraphicsAdapter.Adapters[ i ];
                    type = D3D.Graphics.DeviceType.Reference;
                }
            }
#endif
            D3D.Graphics.PresentationParameters presentParams = CreatePresentationParams( adapter, isFullscreen, depthBuffer, width, height, colorDepth );
            ...
            try
            {
                ...
                newDevice = new D3D.Graphics.GraphicsDevice( adapter, type, target.Handle, D3D.Graphics.CreateOptions.HardwareVertexProcessing, presentParams );
            ...
            catch ( Exception )
            {
                try
                {
                    ...
                    newDevice = new D3D.Graphics.GraphicsDevice( adapter, D3D.Graphics.DeviceType.Hardware, target.Handle, D3D.Graphics.CreateOptions.MixedVertexProcessing, presentParams );
                }
                catch ( Exception )
                {
                    ...
                    newDevice = new D3D.Graphics.GraphicsDevice( adapter, D3D.Graphics.DeviceType.Hardware, target.Handle, D3D.Graphics.CreateOptions.SoftwareVertexProcessing, presentParams );
                }
            }
        ...
        private static D3D.Graphics.PresentationParameters CreatePresentationParams( D3D.Graphics.GraphicsAdapter adapter, bool isFullscreen, bool depthBuffer, int width, int height, int colorDepth )
        {
            // if this is the first window, get the device and do other initialization
            // CMH - 4/24/2004 start
            /// get the Direct3D.Device params
            D3D.Graphics.PresentationParameters presentParams = new D3D.Graphics.PresentationParameters();
            ...
            presentParams.MultiSampleType = D3D.Graphics.MultiSampleType.None;
            presentParams.SwapEffect = D3D.Graphics.SwapEffect.Discard;
            // TODO: Check vsync setting
            presentParams.PresentationInterval = D3D.Graphics.PresentInterval.Immediate;

            // supports 16 and 32 bit color
            if ( colorDepth == 16 )
            {
                presentParams.BackBufferFormat = D3D.Graphics.SurfaceFormat.Bgr565;
            }
            else
            {
                presentParams.BackBufferFormat = D3D.Graphics.SurfaceFormat.Bgr32;
            }

            if ( colorDepth > 16 )
            {
                // check for 24 bit Z buffer with 8 bit stencil (optimal choice)
                if ( !D3D.Graphics.GraphicsAdapter.Adapters[ 0 ].CheckDeviceFormat( D3D.Graphics.DeviceType.Hardware, presentParams.BackBufferFormat, D3D.Graphics.ResourceUsage.DepthStencil, D3D.Graphics.QueryUsage.None, D3D.Graphics.ResourceType.Surface, D3D.Graphics.DepthFormat.Depth24Stencil8 ) )
                {
                    // doh, check for 32 bit Z buffer then
                    if ( !D3D.Graphics.GraphicsAdapter.Adapters[ 0 ].CheckDeviceFormat( D3D.Graphics.DeviceType.Hardware, presentParams.BackBufferFormat, D3D.Graphics.ResourceUsage.DepthStencil, D3D.Graphics.QueryUsage.None, D3D.Graphics.ResourceType.Surface, D3D.Graphics.DepthFormat.Depth32 ) )
                    {
                        // float doh, just use 16 bit Z buffer
                        presentParams.AutoDepthStencilFormat = D3D.Graphics.DepthFormat.Depth16;
                    }
                    else
                    {
                        // use 32 bit Z buffer
                        presentParams.AutoDepthStencilFormat = D3D.Graphics.DepthFormat.Depth32;
                    }
                }
                else
                {
                    if ( adapter.CheckDepthStencilMatch( D3D.Graphics.DeviceType.Hardware, presentParams.BackBufferFormat, presentParams.BackBufferFormat, D3D.Graphics.DepthFormat.Depth24Stencil8 ) )
                    {
                        presentParams.AutoDepthStencilFormat = D3D.Graphics.DepthFormat.Depth24Stencil8;
                    }
                    else
                    {
                        presentParams.AutoDepthStencilFormat = D3D.Graphics.DepthFormat.Depth24Stencil8Single;
                    }
                }
            }
            else
        ...
        public override SceneDetailLevel RasterizationMode
        {
            get
            {
                ...
                {
                    case D3D.Graphics.FillMode.Point:
                        return SceneDetailLevel.Points;
                    case D3D.Graphics.FillMode.WireFrame:
                        return SceneDetailLevel.Wireframe;
                    case D3D.Graphics.FillMode.Solid:
            ...
            set
            {
                switch ( value )
                {
                    case SceneDetailLevel.Points:
                        device.RenderState.FillMode = D3D.Graphics.FillMode.Point;
                        break;
                    case SceneDetailLevel.Wireframe:
                        device.RenderState.FillMode = D3D.Graphics.FillMode.WireFrame;
                        break;
                    case SceneDetailLevel.Solid:
                        device.RenderState.FillMode = D3D.Graphics.FillMode.Solid;
                        break;
                }
            }
        }
        ...
        {
            D3D.Graphics.ColorWriteEnable val = 0;

            if ( red )
            {
                val |= D3D.Graphics.ColorWriteEnable.Red;
            }
            if ( green )
            {
                val |= D3D.Graphics.ColorWriteEnable.Green;
            }
            if ( blue )
            {
                val |= D3D.Graphics.ColorWriteEnable.Blue;
            }
            if ( alpha )
            {
                val |= D3D.Graphics.ColorWriteEnable.Alpha;
            }
        ...
        public override void SetFog( FogMode mode, ColorEx color, float density, float start, float end )
        {
            ...
            else
            {
                ...
                device.RenderState.FogTableMode = D3D.Graphics.FogMode.None;
                device.RenderState.FogColor = new D3D.Graphics.Color( (byte)color.r, (byte)color.g, (byte)color.b, (byte)color.a );
        ...
        public override Matrix4 MakeProjectionMatrix( float fov, float aspectRatio, float near, float far, bool forGpuProgram )
        {
            ...
            float h = 1 / Utility.Tan( (float)theta );
        ...
        public override void SetViewport( Axiom.Core.Viewport viewport )
        {
            if ( activeViewport != viewport || viewport.IsUpdated )
            {
                ...
                D3D.Graphics.Surface back = (D3D.Graphics.Surface)activeRenderTarget.GetCustomAttribute( "D3DBACKBUFFER" );
                ...
                D3D.Graphics.Surface depth = (D3D.Graphics.Surface)activeRenderTarget.GetCustomAttribute( "D3DZBUFFER" );
                ...
                D3D.Graphics.Viewport d3dvp = new D3D.Graphics.Viewport();
        ...
        public override void SetTexture( int stage, bool enabled, string textureName )
        {
            ...
            {
                device.Textures[ stage] = texture.DXTexture ;
        ...
        public override void SetTextureLayerAnisotropy( int stage, int maxAnisotropy )
        {
            ...
            if ( device.SamplerStates[ stage ].MaxAnisotropy != maxAnisotropy )
            {
                device.SamplerStates[ stage ].MaxAnisotropy = maxAnisotropy;
            }
        }
        ...
        public override bool DepthWrite
        {
            get
            {
                return device.RenderState.DepthBufferWriteEnable;
            }
            set
            {
                device.RenderState.DepthBufferWriteEnable = value;
            }
        }
        ...
        private void SetD3DLight( int index, Axiom.Core.Light light )
        {
            ...
            //            device.Lights[ index ].LightType = D3D.Graphics.LightType.Point;
            //            break;
            //        case LightType.Directional:
            //            device.Lights[ index ].LightType = D3D.Graphics.LightType.Directional;
            //            break;
            //        case LightType.Spotlight:
            //            device.Lights[ index ].LightType = D3D.Graphics.LightType.Spot;
            //            device.Lights[ index ].Falloff = light.SpotlightFalloff;
            //            device.Lights[ index ].InnerConeAngle = (float)( new float( light.SpotlightInnerAngle ).Infloats );
            //            //MathUtil.floatsTofloats( light.SpotlightInnerAngle );
            //            device.Lights[ index ].OuterConeAngle = (float)( new float( light.SpotlightOuterAngle ).Infloats );
            //            //MathUtil.floatsTofloats( light.SpotlightOuterAngle );
            //            break;
            //    } // switch
        ...
        /// <param name="b"></param>
        /// <returns></returns>
        private bool CompareVertexDecls( D3D.Graphics.VertexElement[] a, D3D.Graphics.VertexElement[] b )
        {
            ...
            {
                if ( a[ i ].VertexElementFormat != b[ i ].VertexElementFormat ||
                    a[ i ].Offset != b[ i ].Offset ||
                    a[ i ].Stream != b[ i ].Stream ||
                    a[ i ].VertexElementMethod != b[ i ].VertexElementMethod ||
                    a[ i ].VertexElementUsage != b[ i ].VertexElementUsage ||
        ...
        /// </summary>
        private void CheckCaps( D3D.Graphics.GraphicsDevice device )
        {
            ...
            if ( d3dCaps.DeclarationTypes.SupportsByte4 )
            {
            ...
            if ( caps.CheckCap( Capabilities.VertexPrograms ) )
            {
                ...
                D3D.Graphics.GraphicsAdapter details = D3D.Graphics.GraphicsAdapter.Adapters[ 0 ];
    ...
    internal struct XNATextureStageDesc
    {
        ...
        /// texture 
        public D3D.Graphics.Texture tex;
    }
