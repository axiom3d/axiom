...
namespace Axiom.RenderSystems.XNA
{
    ...
    public class XNATexture : Axiom.Core.Texture
    {
        ...
        /// </summary>
        private D3D.Graphics.Texture texture;
        /// <summary>
        ///     1D/2D normal texture.
        /// </summary>
        private D3D.Graphics.Texture2D normTexture;
        /// <summary>
        ///     Cubic texture reference.
        /// </summary>
        private D3D.Graphics.TextureCube cubeTexture;
        /// <summary>
        ///     Temporary 1D/2D normal texture.
        /// </summary>
        private D3D.Graphics.Texture2D tempNormTexture;
        /// <summary>
        ///     Temporary cubic texture reference.
        /// </summary>
        private D3D.Graphics.TextureCube tempCubeTexture;
        /// <summary>
        ///     3D volume texture.
        /// </summary>
        private D3D.Graphics.Texture3D volumeTexture;
        /// <summary>
        ///     Render surface depth/stencil buffer. 
        /// </summary>
        private D3D.Graphics.Surface depthBuffer;
        /// <summary>
        ///     Back buffer pixel format.
        /// </summary>
        private D3D.Graphics.ImageFileFormat bbPixelFormat;
        /// <summary>
        ///     Direct3D device creation parameters.
        /// </summary>
        private D3D.Graphics.GraphicsDeviceCreationParameters devParms;
        /// <summary>
        ///     Direct3D device capability structure.
        /// </summary>
        private D3D.Graphics.GraphicsDeviceCapabilities devCaps;
        /// <summary>
        ...
        public XNATexture( string name, D3D.Graphics.GraphicsDevice device, TextureType type, int width, int height, int numMipMaps, PixelFormat format, TextureUsage usage )
        {
            ...
            using ( D3D.Graphics.Surface back = device.GetBackBuffer( 0, 0 ) )
            {
                bbPixelFormat = (D3D.Graphics.ImageFileFormat)back.Format;
            }
        ...
        /// </summary>
        public D3D.Graphics.Texture DXTexture
        {
            get
            {
                return texture;
        ...
        public D3D.Graphics.Texture2D NormalTexture
        {
            get
            {
                return normTexture;
        ...
        public D3D.Graphics.TextureCube CubeTexture
        {
            get
            {
                return cubeTexture;
        ...
        public D3D.Graphics.Texture3D VolumeTexture
        {
            get
            {
        ...
        public D3D.Graphics.Surface DepthStencil
        {
            get
            {
                return depthBuffer;
        ...
        private void LoadNormalTexture()
        {
            ...
            D3D.Graphics.Surface desc = normTexture.GetSurfaceLevel( 0 );
        ...
        private void LoadVolumeTexture()
        {
            ...
            // set src and dest attributes to the same, we can't know
            D3D.Graphics.Texture3DVolume desc = volumeTexture.GetVolumeLevel( 0 );
        ...
        private void CreateCubeTexture()
        {
            ...
            D3D.Graphics.ImageFileFormat d3dPixelFormat =
                ( usage == TextureUsage.RenderTarget ) ? bbPixelFormat : (D3D.Graphics.ImageFileFormat)ChooseD3DFormat();

            // set the appropriate usage based on the usage of this texture
            D3D.Graphics.ResourceUsage d3dUsage =
                ( usage == TextureUsage.RenderTarget ) ? D3D.Graphics.ResourceUsage.RenderTarget : 0;
            ...
            {
                if ( this.CanAutoGenMipMaps( d3dUsage, D3D.Graphics.ResourceType.TextureCube, d3dPixelFormat ) )
                {
                    d3dUsage |= D3D.Graphics.ResourceUsage.AutoGenerateMipMap;
                    numMips = 0;
                }
            }
            else
            ...
            if ( d3dPixelFormat == D3D.Graphics.ImageFileFormat.R8G8B8 )
            {
                d3dPixelFormat = D3D.Graphics.SurfaceFormat.Color;
            }
        ...
        private void CopyMemoryToSurface( byte[] buffer, D3D.Graphics.Surface surface )
        {
        ...
        private void GetColorMasks( D3D.Graphics.SurfaceFormat format, out uint red, out uint green, out uint blue, out uint alpha, out uint rgbBitCount )
        {
            // we choose the format of the D3D texture so check only for our pf types...
            switch ( format )
            {
                case D3D.Graphics.SurfaceFormat.X8R8G8B8:
                    red = 0x00FF0000;
                    green = 0x0000FF00;
                    blue = 0x000000FF;
                    alpha = 0x00000000;
                    rgbBitCount = 32;
                    break;
                case D3D.Graphics.SurfaceFormat.R8G8B8:
                    red = 0x00FF0000;
                    green = 0x0000FF00;
                    blue = 0x000000FF;
                    alpha = 0x00000000;
                    rgbBitCount = 24;
                    break;
                case D3D.Graphics.SurfaceFormat.Color:
                    red = 0x00FF0000;
                    green = 0x0000FF00;
                    blue = 0x000000FF;
                    alpha = 0xFF000000;
                    rgbBitCount = 32;
                    break;
                case D3D.Graphics.SurfaceFormat.X1R5G5B5:
                    red = 0x00007C00;
                    green = 0x000003E0;
                    blue = 0x0000001F;
                    alpha = 0x00000000;
                    rgbBitCount = 16;
                    break;
                case D3D.Graphics.SurfaceFormat.R5G6B5:
                    red = 0x0000F800;
                    green = 0x000007E0;
                    blue = 0x0000001F;
                    alpha = 0x00000000;
                    rgbBitCount = 16;
                    break;
                case D3D.Graphics.SurfaceFormat.A4R4G4B4:
                    red = 0x00000F00;
                    green = 0x000000F0;
                    blue = 0x0000000F;
                    alpha = 0x0000F000;
        ...
        private D3D.Graphics.TextureFilter GetBestFilterMethod()
        {
            // TODO : do it really :)
        ...
        /// <returns></returns>
        private bool CanAutoGenMipMaps( D3D.Graphics.ResourceUsage srcUsage, D3D.Graphics.ResourceType srcType, D3D.Graphics.ImageFileFormat srcFormat )
        {
            ...
            if ( device.Capabilities.DriverCaps.CanAutoGenerateMipMap )
            {
                ...
                return D3D.Graphics.GraphicsAdapter.Adapters[0].CheckDeviceFormat(
                    devParms.AdapterOrdinal,
                    devParms.DeviceType,
                    bbPixelFormat,
                    srcUsage | D3D.Graphics.ResourceUsage.AutoGenerateMipMap,
                    srcType,
                    srcFormat );
            }
        ...
        private D3D.Graphics.SurfaceFormat ChooseD3DFormat()
        {
            if ( finalBpp > 16 && hasAlpha )
            {
                return D3D.Graphics.SurfaceFormat.Color;
            }
            else if ( finalBpp > 16 && !hasAlpha )
            {
                return D3D.Graphics.SurfaceFormat.X8R8G8B8;
            }
            else if ( finalBpp == 16 && hasAlpha )
            {
                return D3D.Graphics.SurfaceFormat.A4R4G4B4;
            }
            else if ( finalBpp == 16 && !hasAlpha )
            {
                return D3D.Graphics.SurfaceFormat.R5G6B5;
            }
            else
            {
        ...
        /// <param name="format"></param>
        /// <returns></returns>
        public PixelFormat ConvertFormat( D3D.Graphics.SurfaceFormat format )
        {
            switch ( format )
            {
                case D3D.Graphics.SurfaceFormat.A8:
                    return PixelFormat.A8;
                case D3D.Graphics.SurfaceFormat.A4L4:
                    return PixelFormat.A4L4;
                case D3D.Graphics.SurfaceFormat.A4R4G4B4:
                    return PixelFormat.A4R4G4B4;
                case D3D.Graphics.SurfaceFormat.Color:
                    return PixelFormat.A8R8G8B8;
                case D3D.Graphics.SurfaceFormat.A2R10G10B10:
                    return PixelFormat.A2R10G10B10;
                case D3D.Graphics.SurfaceFormat.L8:
        ...
        /// <param name="format"></param>
        /// <returns></returns>
        public D3D.Graphics.SurfaceFormat ConvertFormat( PixelFormat format )
        {
            switch ( format )
            {
                case PixelFormat.L8:
                    return D3D.Graphics.SurfaceFormat.Luminance8;
                case PixelFormat.A8:
                    return D3D.Graphics.SurfaceFormat.A8;
                ...
                case PixelFormat.R8G8B8:
                    return D3D.Graphics.SurfaceFormat.R8G8B8;
                case PixelFormat.B8G8R8A8:
                case PixelFormat.A8R8G8B8:
                    return D3D.Graphics.SurfaceFormat.Color;
                case PixelFormat.L4A4:
                case PixelFormat.A4L4:
                    return D3D.Graphics.SurfaceFormat.A4L4;
                case PixelFormat.B10G10R10A2:
                case PixelFormat.A2R10G10B10:
                    return D3D.Graphics.SurfaceFormat.A2R10G10B10;
            }

            return D3D.Graphics.SurfaceFormat.Unknown;
        }
