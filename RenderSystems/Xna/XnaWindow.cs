...
namespace Axiom.RenderSystems.XNA
{
    ...
    public class D3DWindow : RenderWindow
    {
        ...
        private D3D.Graphics.Surface backBuffer;
        private D3D.Graphics.Surface stencilBuffer;
        private D3D.Graphics.SwapChain swapChain;
        ...
        public override void Create( string name, int width, int height, int colorDepth, bool isFullScreen, int left, int top, bool depthBuffer, params object[] miscParams )
        {
            ...
            {
                device = (D3D.Graphics.GraphicsDevice)miscParams[1];
            }
            if ( device == null )
            {
            ...
            else
            {
                ...
                D3D.Graphics.PresentationParameters presentParameters = device.GetSwapChain( 0 ).PresentationParameters.Clone();

                presentParameters.IsFullScreen = false;
                presentParameters.BackBufferCount = 1;
                presentParameters.EnableAutoDepthStencil = depthBuffer;
                presentParameters.SwapEffect = D3D.Graphics.SwapEffect.Discard;
                presentParameters.DeviceWindowHandle = targetControl.Handle;
                presentParameters.BackBufferHeight = height;
                presentParameters.BackBufferWidth = width;
                swapChain = new D3D.Graphics.SwapChain( device, presentParameters );
        ...
        public override void Resize( int width, int height )
        {
            ...
            if ( !isFullScreen )
            {
                ...
                D3D.Graphics.PresentationParameters p = swapChain.PresentationParameters.Clone();
                p.BackBufferHeight = height;
                p.BackBufferWidth = width;
                swapChain.Dispose();
                swapChain = new D3D.Graphics.SwapChain( device, p );
                stencilBuffer.Dispose();
                stencilBuffer = device.CreateDepthStencilSurface(
                    width, height,
        ...
        public override void SwapBuffers( bool waitForVSync )
        {
            ...
            catch ( D3D.Graphics.DeviceLostException dlx )
            {
                Console.WriteLine( dlx.ToString() );
            }
            catch ( D3D.Graphics.DeviceNotResetException dnrx )
            {
                Console.WriteLine( dnrx.ToString() );
                device.Reset( device.GetSwapChain(0).PresentationParameters );
            }
        }
        ...
        {
            //D3D.Graphics.DisplayMode mode = device.DisplayMode;

            //D3D.Graphics.Surface desc = new D3D.Graphics.Surface();
            //desc.Width = mode.Width;
            //desc.Height = mode.Height;
            //desc.Format = D3D.Graphics.SurfaceFormat.Color;

            //// create a temp surface which will hold the screen image
            //D3D.Graphics.Surface surface = device.CreateOffscreenPlainSurface(
            //    mode.Width, mode.Height, D3D.Graphics.SurfaceFormat.Color, D3D.Pool.SystemMemory );
            ...
            //    D3D.Surface tmpSurface = device.CreateOffscreenPlainSurface( rect.Width, rect.Height, D3D.Graphics.SurfaceFormat.Color, D3D.Pool.Default );
        ...
        {
            D3D.Graphics.GraphicsDevice resetDevice = (D3D.Graphics.GraphicsDevice)sender;

            // Turn off culling, so we see the front and back of the triangle
            resetDevice.RenderState.CullMode = D3D.Graphics.CullMode.None;
            // Turn on the ZBuffer
