	...
	public sealed class PlatformManager {
		...
		private static string SelectPlatformManagerAssembly(string[] files) {
			...
			if (files.Length == 0) {
				...
				if(windows){	//if this is a Microsoft OS such as Windows or Win32S
					for(int i = 0;i<files.Length; i++) {
						if((files[i].IndexOf("Win32") != -1) == windows ) {//use win32 for windows or another platforms (such as SDL) for non-windows
							return files[i];
						}
					}
					if(windows) {
