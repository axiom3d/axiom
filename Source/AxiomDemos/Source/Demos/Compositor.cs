	...
	public partial class Compositor : TechDemo
	{
		...
		protected override void OnFrameStarted( object source, FrameEventArgs evt )
		{
			...
			if ( input.IsKeyPressed( KeyCodes.Space ) )
			{
				...
				{
					CompositorManager.Instance.SetCompositorEnabled( this.window.GetViewport( 0 ), _compositorList[ _compositorIndex ], false );
				}

				_compositorIndex = ++_compositorIndex % _compositorList.Length;

				CompositorManager.Instance.SetCompositorEnabled( this.window.GetViewport( 0 ), _compositorList[ _compositorIndex ], true );
			}

			if ( input.IsKeyPressed( KeyCodes.D1 ) )
			{
				CompositorManager.Instance.SetCompositorEnabled( this.window.GetViewport( 0 ), _compositorList[ 0 ], !_compositorEnabled[ 0 ] );
			...
			{
				CompositorManager.Instance.SetCompositorEnabled( this.window.GetViewport( 0 ), _compositorList[ 1 ], !_compositorEnabled[ 1 ] );
		...
		public override void CreateScene()
		{
			...
			//                                                 true);
			CompositorManager.Instance.SetCompositorEnabled( this.window.GetViewport( 0 ), _compositorList[ 0 ], true );
		}
			...
			public void SetCompositor( CompositorInstance compositor )
			{
				...
				foreach ( CompositionTechnique.TextureDefinition textureDefinition in compositor.Technique.TextureDefinitions )
				{
					if ( textureDefinition.Name == "rt_bloom0" )
					{
						...
						{
							int offset = i * 4;
							bloomTexWeights[ offset + 0 ] = bloomTexWeights[ offset + 1 ] = bloomTexWeights[ offset + 2 ] = bloomTexWeights[ offset - 7 * 4 + 0 ];
							bloomTexWeights[ offset + 3 ] = 1.0f;
