...
namespace Axiom.Graphics
{
	...
	/// </summary>
	/// <remarks>
	///   The RenderSystem class provides a base class which abstracts the general functionality of the 3D API e.g. Direct3D or OpenGL. Whilst a few of the general methods have implementations, most of this class is abstract, requiring a subclass based on a specific API to be constructed to provide the full functionality. <p /> Note there are 2 levels to the interface - one which will be used often by the caller of the engine library, and one which is at a lower level and will be used by the other classes provided by the engine. These lower level methods are marked as internal, and are not accessible outside of the Core library.
	/// </remarks>
	public abstract class RenderSystem : DisposableObject
	{
		...
		/// </summary>
		/// <remarks>
		///   A typical way for the rendering engine to cull triangles is based on the 'vertex winding' of triangles. Vertex winding refers to the direction in which the vertices are passed or indexed to in the rendering operation as viewed from the camera, and will wither be clockwise or counterclockwise. The default is <see
		///    name="CullingMode.Clockwise" /> i.e. that only triangles whose vertices are passed/indexed in counterclockwise order are rendered - this is a common approach and is used in 3D studio models for example. You can alter this culling mode if you wish but it is not advised unless you know what you are doing. You may wish to use the <see
		///    cref="Graphics.CullingMode.None" /> option for mesh data that you cull yourself where the vertex winding is uncertain.
		/// </remarks>
		[OgreVersion( 1, 7, 2790 )]
		...
		/// <summary>
		///   Sets the comparison function for the depth buffer check. Advanced use only - allows you to choose the function applied to compare the depth values of new and existing pixels in the depth buffer. Only an issue if the depth buffer check is enabled <see
		///    cref="DepthBufferCheckEnabled" />
		/// </summary>
		...
		/// </summary>
		/// <remarks>
		///   Set the current multi pass count value. This must be set prior to calling render() if multiple renderings of the same pass state are required.
		/// </remarks>
		[OgreVersion( 1, 7, 2790 )]
		...
		/// <summary>
		///   Gets/Sets whether or not dynamic lighting is enabled. <p /> If true, dynamic lighting is performed on geometry with normals supplied, geometry without normals will not be displayed. If false, no lighting is applied and all geometry will be full brightness.
		/// </summary>
		[OgreVersion( 1, 7, 2790 )]
		...
		/// </summary>
		/// <remarks>
		///   This is useful when, for example, you are scaling SceneNodes such that normals may not be unit-length anymore. Note though that this has an overhead so should not be turn on unless you really need it. <p /> You should not normally call this direct unless you are rendering world geometry; set it on the Renderable because otherwise it will be overridden by material settings.
		/// </remarks>
		[OgreVersion( 1, 7, 2790 )]
		...
		/// </summary>
		/// <remarks>
		///   When using identity transforms you can manually set the depth of a vertex; however the input values required differ per rendersystem. This method lets you retrieve the correct value. <see
		///    cref="SimpleRenderable.UseIdentityView" /> <see cref="SimpleRenderable.UseIdentityProjection" />
		/// </remarks>
		[OgreVersion( 1, 7, 2790 )]
		...
		/// </summary>
		/// <remarks>
		///   Call this from your main thread before starting your other threads (which themselves should call registerThread()). Note that if you start your own threads, there is a specific startup sequence which must be respected and requires synchronisation between the threads: <ol>
		///                                                                                                                                                                                                                                                                                 <li>[Main thread]Call
		...
		///                                                                                                                                                                                                                                                                                 </li>
		///                                                                                                                                                                                                                                                                               </ol> Once this init sequence is completed the threads are independent but this startup sequence must be respected.
		/// </remarks>
		[OgreVersion( 1, 7, 2790 )]
		...
		/// <summary>
		///   Tell the rendersystem to perform any tasks it needs to directly after other threads which might access the rendering API are registered. <see
		///    cref="PreExtraThreadsStarted" />
		/// </summary>
		[OgreVersion( 1, 7, 2790 )]
		...
		/// <summary>
		///   Unregister an additional thread which may make calls to rendersystem-related objects. <see cref="RegisterThread" />
		/// </summary>
		[OgreVersion( 1, 7, 2790 )]
		...
		/// </summary>
		/// <remarks>
		///   Low-level rendering interface to perform rendering operations. Unlikely to be used directly by client applications, since the <see
		///    cref="SceneManager" /> and various support classes will be responsible for calling this method. Can only be called between BeginScene and EndScene
		/// </remarks>
		...
		/// <summary>
		///   Internal method for swapping all the buffers on all render targets, if <see cref="UpdateAllRenderTargets( bool )" /> was called with a 'false' parameter.
		/// </summary>
		[OgreVersion( 1, 7, 2790 )]
		...
		/// <summary>
		///   Pause rendering for a frame. This has to be called after <see cref="BeginFrame" /> and before <see cref="EndFrame" /> . will usually be called by the SceneManager, don't use this manually unless you know what you are doing.
		/// </summary>
		[OgreVersion( 1, 7, 2790 )]
		...
		/// <summary>
		///   Resume rendering for a frame. This has to be called after a <see cref="PauseFrame" /> call Will usually be called by the SceneManager, don't use this manually unless you know what you are doing.
		/// </summary>
		...
		/// <summary>
		///   Converts the Axiom.Core.ColorEx value to a int. Each API may need the bytes of the packed color data in different orders. i.e. OpenGL - ABGR, D3D - ARGB
		/// </summary>
		...
		/// <summary>
		///   Requests an API implementation of a hardware occlusion query used to test for the number of fragments rendered between calls to <see
		///    cref="HardwareOcclusionQuery.Begin" /> and <see cref="HardwareOcclusionQuery.End" /> that pass the depth buffer test.
		/// </summary>
		...
		/// </summary>
		/// <remarks>
		///   Because different APIs have different requirements (some incompatible) for the projection matrix, this method allows each to implement their own correctly and pass back a generic Matrix4 for storage in the engine.
		/// </remarks>
		...
		/// <summary>
		///   Used to confirm the settings (normally chosen by the user) in order to make the renderer able to inialize with the settings as required. This make be video mode, D3D driver, full screen / windowed etc. Called automatically by the default configuration dialog, and by the restoration of saved settings. These settings are stored and only activeated when RenderSystem::Initalize or RenderSystem::Reinitialize are called
		/// </summary>
		...
		/// </summary>
		/// <remarks>
		///   For some advanced effects, you may wish to turn off the writing of certain color channels, or even all of the color channels so that only the depth buffer is updated in a rendering pass. However, the chances are that you really want to use this option through the Material class.
		/// </remarks>
		...
		/// </summary>
		/// <remarks>
		///   Sometimes you may wish to alter the behavior of the depth buffer to achieve special effects. Because it's unlikely that you'll set these options for an entire frame, but rather use them to tweak settings between rendering objects, this is intended for internal uses, which will be used by a <see
		///    cref="SceneManager" /> implementation rather than directly from the client application.
		/// </remarks>
		...
		/// <summary>
		///   Sets the global blending factors for combining subsequent renders with the existing frame contents. The result of the blending operation is: <p
		///    align="center">final = (texture * src) + (pixel * dest)</p> Each of the factors is specified as one of a number of options, as specified in the SceneBlendFactor enumerated type.
		/// </summary>
		...
		/// </summary>
		/// <remarks>
		///   This method allows you to 'mask off' rendering in all but a given rectangular area as identified by the parameters to this method. <p /> Not all systems support this method. Check the <see
		///    cref="Capabilities" /> enum for the ScissorTest capability to see if it is supported.
		/// </remarks>
		...
		///     The stencil test is:
		///     <PRE>(Reference Value &amp; Mask) CompareFunction (Stencil Buffer Value &amp; Mask)</PRE>
		...
		///     depth buffer check passing too.</para> <para>Unlike other render states, stencilling is left for the application to turn
		...
		///                                              between batches.</para> <para>There are individual state change methods for each of the parameters set using 
		///                                                                        this method. 
		...
		/// <summary>
		///   Sets the surface properties to be used for future rendering. This method sets the the properties of the surfaces of objects to be rendered after it. In this context these surface properties are the amount of each type of light the object reflects (determining it's color under different types of light), whether it emits light itself, and how shiny it is. Textures are not dealt with here, <see
		///    cref="SetTexture(int, bool, Texture)" /> method for details. This method is used by SetMaterial so does not need to be called direct if that method is being used.
		/// </summary>
		...
		/// <summary>
		///   Sets the details of a texture stage, to be used for all primitives rendered afterwards. User processes would not normally call this direct unless rendering primitives themselves - the SubEntity class is designed to manage materials for objects. Note that this method is called by SetMaterial.
		/// </summary>
		...
		/// </summary>
		/// <remarks>
		///   Not all rendersystems support separate vertex samplers. For those that do, you can set a texture for them, separate to the regular texture samplers, using this method. For those that don't, you should use the regular texture samplers which are shared between the vertex and fragment units; calling this method will throw an exception. <see
		///    cref="RenderSystemCapabilities.VertexTextureUnitsShared" />
		/// </remarks>
		...
		/// <summary>
		///   Sets the texture matrix for the specified stage. Used to apply rotations, translations, and scaling to textures.
		/// </summary>
		[OgreVersion( 1, 7, 2790 )]
