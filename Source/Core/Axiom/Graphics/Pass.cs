...
namespace Axiom.Graphics
{
	...
	public class Pass : DisposableObject
	{
		...
		/// </summary>
		/// <remarks>
		///   The base color of a pass is determined by how much red, green and blue light is reflects (provided texture layer #0 has a blend mode other than LayerBlendOperation.Replace). This property determines how much ambient light (directionless global light) is reflected. The default is full white, meaning objects are completely globally illuminated. Reduce this if you want to see diffuse or specular light effects, or change the blend of colors to make the object have a base color other than white. <p /> This setting has no effect if dynamic lighting is disabled (see <see
		///    cref="Pass.LightingEnabled" /> ), or if this is a programmable pass.
		/// </remarks>
		...
		/// </summary>
		/// <remarks>
		///   The base color of a pass is determined by how much red, green and blue light is reflects (provided texture layer #0 has a blend mode other than LayerBlendOperation.Replace). This property determines how much diffuse light (light from instances of the Light class in the scene) is reflected. The default is full white, meaning objects reflect the maximum white light they can from Light objects. <p /> This setting has no effect if dynamic lighting is disabled (see <see
		///    cref="Pass.LightingEnabled" /> ), or if this is a programmable pass.
		/// </remarks>
		...
		/// </summary>
		/// <remarks>
		///   The base color of a pass is determined by how much red, green and blue light is reflects (provided texture layer #0 has a blend mode other than LBO_REPLACE). This property determines how much specular light (highlights from instances of the Light class in the scene) is reflected. The default is to reflect no specular light. <p /> The size of the specular highlights is determined by the separate Shininess property. <p /> This setting has no effect if dynamic lighting is disabled (see <see
		///    cref="Pass.LightingEnabled" /> ), or if this is a programmable pass.
		/// </remarks>
		...
		/// </summary>
		/// <remarks>
		///   If depth-buffer checking is on, whenever a pixel is about to be written to the frame buffer the depth buffer is checked to see if the pixel is in front of all other pixels written at that point. If not, the pixel is not written. <p /> If depth checking is off, pixels are written no matter what has been rendered before. Also see <see
		///    cref="DepthFunction" /> for more advanced depth check configuration.
		/// </remarks>
		...
		/// </summary>
		/// <remarks>
		///   If depth checking is enabled (see <see cref="DepthCheck" /> ) a comparison occurs between the depth value of the pixel to be written and the current contents of the buffer. This comparison is normally CompareFunction.LessEqual, i.e. the pixel is written if it is closer (or at the same distance) than the current contents. If you wish, you can change this comparison using this method.
		/// </remarks>
		...
		///   </summary>
		///   <remarks>
		///     When polygons are coplanar, you can get problems with 'depth fighting' (or 'z fighting') where the pixels from the two polys compete for the same screen pixel. This is particularly a problem for decals (polys attached to another surface to represent details such as bulletholes etc.). <p /> A way to combat this problem is to use a depth bias to adjust the depth buffer value used for the decal such that it is slightly higher than the true value, ensuring that the decal appears on top. There are two aspects to the biasing, a constant bias value and a slope-relative biasing value, which varies according to the maximum depth slope relative to the camera, ie: <pre>finalBias = maxSlope * slopeScaleBias + constantBias</pre> Note that slope scale bias, whilst more accurate, may be ignored by old hardware.
		///   </remarks>
		...
		/// </summary>
		/// <remarks>
		///   The default is <see ref="CompareFunction.AlwaysPass" /> i.e. alpha is not used to reject pixels. <para>This option applies in both the fixed function and the programmable pipeline.</para>
		/// </remarks>
		...
		/// </summary>
		/// <remarks>
		///   A typical way for the rendering engine to cull triangles is based on the 'vertex winding' of triangles. Vertex winding refers to the direction in which the vertices are passed or indexed to in the rendering operation as viewed from the camera, and will wither be clockwise or counterclockwise. The default is Clockwise i.e. that only triangles whose vertices are passed/indexed in counter-clockwise order are rendered - this is a common approach and is used in 3D studio models for example. You can alter this culling mode if you wish but it is not advised unless you know what you are doing. <p /> You may wish to use the CullingMode.None option for mesh data that you cull yourself where the vertex winding is uncertain.
		/// </remarks>
		...
		/// </summary>
		/// <remarks>
		///   The default for this setting is 1, so if you enable light iteration ( <see cref="IteratePerLight" /> ), the pass is rendered once per light. If you set this value higher, the passes will occur once per 'n' lights. The start of the iteration is set by <see
		///    cref="StartLight" /> and the end by <see cref="MaxSimultaneousLights" /> .
		/// </remarks>
		...
		/// </summary>
		/// <remarks>
		///   Only applicable to programmable passes, and this particular call is designed for low-level programs; use the named parameter methods for setting high-level programs. <p /> This must have been created using GpuProgramManager by the time that this Pass is loaded.
		/// </remarks>
		...
		/// </summary>
		/// <remarks>
		///   This unit is is added on top of all previous texture units. <p /> Applies to both fixed-function and programmable passes.
		/// </remarks>
		/// <param name="textureName"> The basic name of the texture (i.e. brickwall.jpg) </param>
		...
		/// </summary>
		/// <remarks>
		///   Fogging is an effect that is applied as polys are rendered. Sometimes, you want fog to be applied to an entire scene. Other times, you want it to be applied to a few polygons only. This pass-level specification of fog parameters lets you easily manage both. <p /> The SceneManager class also has a SetFog method which applies scene-level fog. This method lets you change the fog behavior for this pass compared to the standard scene-level fog.
		/// </remarks>
		...
		/// </summary>
		/// <remarks>
		///   The default behavior for a pass (when this option is 'false'), is for a pass to be rendered only once, with all the lights which could affect this object set at the same time (up to the maximum lights allowed in the render system, which is typically 8). <p /> Setting this option to 'true' changes this behavior, such that instead of trying to issue render this pass once per object, it is run once <b>per light</b> which can affect this object. In this case, only light index 0 is ever used, and is a different light every time the pass is issued, up to the total number of lights which is affecting this object. This has 2 advantages: <ul>
		...
		///                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                </ul> However, this technique is a lot more expensive, and typically you will want an additional ambient pass, because if no lights are affecting the object it will not be rendered at all, which will look odd even if ambient light is zero (imagine if there are lit objects behind it - the objects silhouette would not show up). Therefore, use this option with care, and you would be well advised to provide a less expensive fallback technique for use in the distance. <p /> Note: The number of times this pass runs is still limited by the maximum number of lights allowed as set in MaxLights, so you will never get more passes than this.
		/// </remarks>
		...
		/// </summary>
		/// <remarks>
		///   Whereas the texture blending operations seen in the TextureUnitState class are concerned with blending between texture layers, this blending is about combining the output of the Pass as a whole with the existing contents of the rendering target. This blending therefore allows object transparency and other special effects. If all passes in a technique have a scene blend, then the whole technique is considered to be transparent. <p /> This method allows you to select one of a number of predefined blending types. If you require more control than this, use the alternative version of this method which allows you to specify source and destination blend factors. <p /> This method is applicable for both the fixed-function and programmable pipelines.
		/// </remarks>
		...
		/// </summary>
		/// <remarks>
		///   Wheras the texture blending operations seen in the TextureUnitState class are concerned with blending between texture layers, this blending is about combining the output of the material as a whole with the existing contents of the rendering target. This blending therefore allows object transparency and other special effects. <p /> This version of the method allows complete control over the blending operation, by specifying the source and destination blending factors. The result of the blending operation is: <span
		///    align="center">final = (texture * sourceFactor) + (pixel * destFactor)</span> <p /> Each of the factors is specified as one of a number of options, as specified in the SceneBlendFactor enumerated type. <p /> This method is applicable for both the fixed-function and programmable pipelines.
		/// </remarks>
		...
		/// </summary>
		/// <remarks>
		///   Wheras the texture blending operations seen in the TextureUnitState class are concerned with blending between texture layers, this blending is about combining the output of the material as a whole with the existing contents of the rendering target. This blending therefore allows object transparency and other special effects. @par This version of the method allows complete control over the blending operation, by specifying the source and destination blending factors. The result of the blending operation is: <span
		///    align="center">final = (texture * sourceFactor) + (pixel * destFactor)</span> @par Each of the factors is specified as one of a number of options, as specified in the SceneBlendFactor enumerated type. @note This method is applicable for both the fixed-function and programmable pipelines.
		/// </remarks>
		...
		///</summary>
		///<remarks>
		///  By default this operation is +, which creates this equation <span align="center">final = (texture * sourceFactor) + (pixel * destFactor)</span> By setting this to something other than SBO_ADD you can change the operation to achieve a different effect. This function allows more control over blending since it allows you to select different blending modes for the color and alpha channels
		///</remarks>
