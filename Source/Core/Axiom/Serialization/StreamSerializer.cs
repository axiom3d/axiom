...
namespace Axiom.Serialization
{
	...
	public class StreamSerializer : DisposableObject
	{
		...
		/// </summary>
		/// <remarks>
		///   Returns the offset. Note that a return value of 0 means that either the position is at the start of the chunk data section (ie right after the header), or that no chunk is currently active. Use getCurrentChunkID or getCurrentChunkDepth to determine if a chunk is active.
		/// </remarks>
		...
		/// </summary>
		/// <remarks>
		///   Files are serialised in a chunk-based manner, meaning that each section of data is prepended by a chunk header. After reading this chunk header, the next set of data is available directly afterwards. <para /> When you have finished with this chunk, you should call readChunkEnd. This will perform a bit of validation and clear the chunk from the stack.
		/// </remarks>
		...
		/// </summary>
		/// <remarks>
		///   The most common case of wanting to use this is if you'd called <see cref="ReadChunkBegin()" /> , but the chunk you read wasn't one you wanted to process, and rather than skipping over it (which <see
		///    cref="ReadChunkEnd" /> would do), you want to backtrack and give something else an opportunity to read it.
		/// </remarks>
		...
		/// </summary>
		/// <remarks>
		///   You can call this method at any point after calling <see cref="ReadChunkBegin()" /> , even if you didn't read all the rest of the data in the chunk. If you did not read to the end of a chunk, this method will automatically skip over the remainder of the chunk and position the stream just after it.
		/// </remarks>
		/// <param name="id"> The id of the chunk that you were reading (for validation purposes) </param>
		[OgreVersion( 1, 7, 2 )]
			...
			if ( !IsDisposed )
			{
				if ( disposeManagedResources )
				{
					lock ( this )
					{
						if ( mChunkStack.Count != 0 )
						{
							Debug.WriteLine( "Warning: stream was not fully read / written; " + mChunkStack.Count +
