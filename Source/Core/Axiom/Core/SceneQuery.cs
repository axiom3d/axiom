...
namespace Axiom.Core
{
	...
	/// </summary>
	/// <remarks>
	///   This is an abstract class for performing a query on a scene, i.e. to retrieve a list of objects and/or world geometry sections which are potentially intersecting a given region. Note the use of the word 'potentially': the results of a scene query are generated based on bounding volumes, and as such are not correct at a triangle level; the user of the SceneQuery is expected to filter the results further if greater accuracy is required. <p /> Different SceneManagers will implement these queries in different ways to exploit their particular scene organization, and thus will provide their own concrete subclasses. In fact, these subclasses will be derived from subclasses of this class rather than directly because there will be region-type classes in between. <p /> These queries could have just been implemented as methods on the SceneManager, however, they are wrapped up as objects to allow 'compilation' of queries if deemed appropriate by the implementation; i.e. each concrete subclass may precalculate information (such as fixed scene partitions involved in the query) to speed up the repeated use of the query. <p /> You should never try to create a SceneQuery object yourself, they should be created using the SceneManager interfaces for the type of query required, e.g. SceneManager.CreateRaySceneQuery.
	/// </remarks>
	public abstract class SceneQuery
	{
		...
		/// </summary>
		/// <remarks>
		///   This property allows you to set a 'mask' to limit the results of this query to certain types of result. The actual meaning of this value is up to the application; basically SceneObject instances will only be returned from this query if a bitwise AND operation between this mask value and the SceneObject.QueryFlags value is non-zero. The application will have to decide what each of the bits means.
		/// </remarks>
		...
		/// </summary>
		/// <remarks>
		///   This property allows you to set a 'mask' to limit the results of this query to certain types of result. Whilst setQueryMask deals with flags set per instance of object, this method deals with setting a mask on flags set per type of object. Both may exclude an object from query results.
		/// </remarks>
	...
	///</summary>
	///<remarks>
	///  This class is simply a generalization of the subtypes of query that return a set of individual results in a region. See the <see
	///   cref="SceneQuery" /> class for abstract information, and subclasses for the detail of each query type.
	///</remarks>
	public abstract class RegionSceneQuery : SceneQuery, ISceneQueryListener
	{
		...
		///</summary>
		///<remarks>
		///  This method executes the scene query as configured, gathers the results into one structure and returns a reference to that structure. These results will also persist in this query object until the next query is executed, or <see
		///   cref="ClearResults" /> is called. An more lightweight version of this method that returns results through a listener is also available.
		///</remarks>
		///<returns> </returns>
	...
	public abstract class RaySceneQuery : SceneQuery, IRaySceneQueryListener
	{
		...
		///</summary>
		///<remarks>
		///  Often you want to know what was the first object a ray intersected with, and this method allows you to ask the query to sort the results so that the nearest results are listed first. <p /> Note that because the query returns results based on bounding volumes, the ray may not actually intersect the detail of the objects returned from the query, just their bounding volumes. For this reason the caller is advised to use more detailed intersection tests on the results if a more accurate result is required; we use bounds checking in order to give the most speedy results since not all applications need extreme accuracy.
		///</remarks>
		...
		///<summary>
		///  Gets/Sets the maximum number of results to return from this query when sorting is enabled.
		///</summary>
		///<remarks>
		...
		///<remarks>
		///  This method executes the scene query as configured, gathers the results into one structure and returns a reference to that structure. These results will also persist in this query object until the next query is executed, or <see
		///   cref="ClearResults" /> . A more lightweight version of this method that returns results through a listener is also available.
		///</remarks>
		///<returns> </returns>
