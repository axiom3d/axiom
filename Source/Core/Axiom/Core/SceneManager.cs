...
namespace Axiom.Core
{
	...
	///</summary>
	///<remarks>
	///  This class defines the interface and the basic behaviour of a 'Scene Manager'. A SceneManager organizes the culling and rendering of the scene, in conjunction with the <see
	///   cref="RenderQueue" /> . This class is designed to be extended through subclassing in order to provide more specialized scene organization structures for particular needs. The default SceneManager culls based on a hierarchy of node bounding boxes, other implementations can use an octree ( <see
	///   name="OctreeSceneManager" /> ), a BSP tree ( <see name="BspSceneManager" /> ), and many other options. New SceneManager implementations can be added at runtime by plugins, see <see
	///   cref="SceneManagerEnumerator" /> for the interfaces for adding new SceneManager types. <p /> There is a distinction between 'objects' (which subclass <see
	///   cref="MovableObject" /> , and are movable, discrete objects in the world), and 'world geometry', which is large, generally static geometry. World geometry tends to influence the SceneManager organizational structure (e.g. lots of indoor static geometry might result in a spatial tree structure) and as such world geometry is generally tied to a given SceneManager implementation, whilst MovableObject instances can be used with any SceneManager. Subclasses are free to define world geometry however they please. <p /> Multiple SceneManager instances can exist at one time, each one with a distinct scene. Which SceneManager is used to render a scene is dependent on the <see
	///   cref="Camera" /> , which will always call back the SceneManager which created it to render the scene.
	///</remarks>
	...
	public abstract class SceneManager : DisposableObject
	{
		...
		///</summary>
		///<remarks>
		///  Note that this does not add the SceneNode to the scene hierarchy. This method is for convenience, since it allows an instance to be created for which the SceneManager is responsible for allocating and releasing memory, which is convenient in complex scenes. <p /> To include the returned SceneNode in the scene, use the AddChild method of the SceneNode which is to be it's parent. <p /> Note that this method takes no parameters, and the node created is unnamed (it is actually given a generated name, which you can retrieve if you want). If you wish to create a node with a specific name, call the alternative method which takes a name parameter.
		///</remarks>
		///<returns> </returns>
		...
		///</summary>
		///<remarks>
		///  Note that this does not add the SceneNode to the scene hierarchy. This method is for convenience, since it allows an instance to be created for which the SceneManager is responsible for allocating and releasing memory, which is convenient in complex scenes. <p /> To include the returned SceneNode in the scene, use the AddChild method of the SceneNode which is to be it's parent. <p /> Note that this method takes a name parameter, which makes the node easier to retrieve directly again later.
		///</remarks>
		///<returns> </returns>
		...
		///</summary>
		///<remarks>
		///  An animation is a collection of 'tracks' which over time change the position / orientation of Node objects. In this case, the animation will likely have tracks to modify the position / orientation of SceneNode objects, e.g. to make objects move along a path. <p /> You don't need to use an Animation object to move objects around - you can do it yourself using the methods of the Node in your application. However, when you need relatively complex scripted animation, this is the class to use since it will interpolate between keyframes for you and generally make the whole process easier to manage. <p /> A single animation can affect multiple Node objects (each AnimationTrack affects a single Node). In addition, through animation blending a single Node can be affected by multiple animations, although this is more useful when performing skeletal animation (see Skeleton.CreateAnimation).
		///</remarks>
		...
		///</summary>
		///<remarks>
		///  <para>You can create Animation objects for animating SceneNode obejcts using the
		...
		///    is provided.</para> <para>So if you don't want to control animation application manually, call this method,
		///                          update the returned object as you like every frame and let SceneManager apply
		///                          the animation state for you.</para> <para>Remember, AnimationState objects are disabled by default at creation time.
		///                                                                Turn them on when you want them using their Enabled property.</para> <para>Note that any SceneNode affected by this automatic animation will have it's state
		...
		///                                                                                                                                       'bake' this reference position into the node.</para>
		///</remarks>
		...
		///    rest of the scene. See the Overlay class for more information.</p> <p>NOTE: after creation, the Overlay is initially hidden. You can create
		...
		/// </summary>
		/// <remarks>
		///   Typically this method returns the origin unless a) world geometry has been loaded using <see
		///    cref="SceneManager.LoadWorldGeometry" /> and b) that world geometry has suggested 'start' points. If there is more than one viewpoint which the scene manager can suggest, it will always suggest the first one unless the random parameter is true.
		/// </remarks>
		...
		///</summary>
		///<remarks>
		///  You could create a sky box yourself using the standard mesh and entity methods, but this creates a plane which the camera can never get closer or further away from - it moves with the camera. (you could create this effect by creating a world box which was attached to the same SceneNode as the Camera too, but this would only apply to a single camera whereas this skybox applies to any camera using this scene manager). <p /> The material you use for the skybox can either contain layers which are single textures, or they can be cubic textures, i.e. made up of 6 images, one for each plane of the cube. See the TextureLayer class for more information.
		///</remarks>
		...
		///</summary>
		///<remarks>
		///  The entire scene is held as a hierarchy of nodes, which allows things like relative transforms, general changes in rendering state etc (See the SceneNode class for more info). In this basic SceneManager class, you are free to structure this hierarchy however you like, since it has no real significance apart from making transforms relative to each node (more specialised subclasses will provide utility methods for building specific node structures e.g. loading a BSP tree).
		///</remarks>
		...
		///<summary>
		///  Method for setting a specific option of the Scene Manager. These options are usually specific for a certain implementation of the Scene Manager class, and may (and probably will) not exist across different implementations.
		///</summary>
		...
		/// <summary>
		///   the default shadow camera setup used for all lights which don't have their own shadow camera setup.
		/// </summary>
		private IShadowCameraSetup _defaultShadowCameraSetup;

		/// <summary>
		///   Get/Set the shadow camera setup in use for all lights which don't have their own shadow camera setup.
		/// </summary>
		...
		///</summary>
		///<remarks>
		///  This is only applicable for shadow techniques which involve darkening the area in shadow, as opposed to masking out the light. This color provided is used as a modulative value to darken the areas.
		///</remarks>
		...
		///</summary>
		///<remarks>
		///  Although directional lights are essentially infinite, there are many reasons to limit the shadow extrusion distance to a finite number, not least of which is compatibility with older cards (which do not support infinite positions), and shadow caster elimination. <p /> The default value is 10,000 world units. This does not apply to point lights or spotlights, since they extrude up to their attenuation range.
		///</remarks>
		...
		///</summary>
		///<remarks>
		///  There are multiple ways to generate shadows in a scene, and each has strengths and weaknesses. <ul>
		...
		///                                                                                                 </ul> <p /> We support 2 kinds of stencil shadows, and 2 kinds of texture-based shadows, and one simple decal approach. The 2 stencil approaches differ in the amount of multipass work that is required - the modulative approach simply 'darkens' areas in shadow after the main render, which is the least expensive, whilst the additive approach has to perform a render per light and adds the cumulative effect, whcih is more expensive but more accurate. The texture based shadows both work in roughly the same way, the only difference is that the shadowmap approach is slightly more accurate, but requires a more recent graphics card. <p /> Note that because mixing many shadow techniques can cause problems, only one technique is supported at once. Also, you should call this method at the start of the scene setup.
		///</remarks>
		...
		///</summary>
		///<remarks>
		///  Stencil shadow coherency is very reliant on the shadow volume not being clipped by the far plane. If this clipping happens, you get a kind of 'negative' shadow effect. The best way to achieve coherency is to move the far plane of the camera out to infinity, thus preventing the far plane from clipping the shadow volumes. When combined with vertex program extrusion of the volume to infinity, which Axiom does when available, this results in very robust shadow volumes. For this reason, when you enable stencil shadows, Ogre automatically changes your camera settings to project to infinity if the card supports it. You can disable this behavior if you like by setting this property; although you can never enable infinite projection if the card does not support it. <p /> If you disable infinite projection, or it is not available, you need to be far more careful with your light attenuation / directional light extrusion distances to avoid clipping artefacts at the far plane. <p /> Recent cards will generally support infinite far plane projection. However, we have found some cases where they do not, especially on Direct3D. There is no standard capability we can check to validate this, so we use some heuristics based on experience: <UL>
		...
		///                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         </UL> Therefore in the RenderSystem implementation, we may veto the use of an infinite far plane based on these heuristics.
		///</remarks>
		...
		///<summary>
		///  Gets/Sets a flag that indicates whether debug shadow info (i.e. visible volumes) will be displayed.
		///</summary>
		...
		///    with the new defaults you have specified.</p> <p>The default settings begin as a single Technique with a single, non-programmable Pass:
		///                                                    <ul>
		...
		/// </summary>
		/// <remarks>
		///   Scene manager tracking lights that affecting the frustum, if changes detected (the changes includes light list itself and the light's position and attenuation range), then increase the lights dirty counter. <para /> When implementing customise lights finding algorithm relied on either <see
		///    cref="lightsAffectingFrustum" /> or <see cref="PopulateLightList" /> , might check this value for sure that the light list are really need to re-populate, otherwise, returns cached light list (if exists) for better performance.
		/// </remarks>
		...
		///    closer than any point lights and as such will always take precedence.</p> <p>Subclasses of the default SceneManager may wish to take into account other issues
		...
		///                                                                                those lights which are out of range.</p> <p>The number of items in the list max exceed the maximum number of lights supported
		...
		/// </summary>
		/// <remarks>
		///   This method should be overridden by SceneManagers that provide custom world geometry that can take some time to load. They should return from this method a count of the number of stages of progress they can report on whilst loading. During real loading (setWorldGeomtry), they should call <see
		///    name="ResourceGroupManager.notifyWorlGeometryProgress" /> exactly that number of times when loading the geometry for real.
		/// </remarks>
