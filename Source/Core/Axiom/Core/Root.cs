...
namespace Axiom.Core
{
	...
	public sealed class Root : IDisposable
	{
		...
		/// <summary>
		///   The number of microseconds per frame when we're limiting frame rates. By default, we don't limit frame rates, and in that case, the number is 0.
		/// </summary>
		...
		///<summary>
		///  Gets a list of all types of SceneManager available for construction, providing some information about each one.
		///</summary>
		...
		/// <summary>
		///   Axiom by default gives you the raw frame time, but can optionally smooth it out over several frames, in order to reduce the noticable effect of occasional hiccups in framerate. These smoothed values are passed back as parameters to FrameListener calls.
		/// </summary>
		/// <remarks>
		///   This method allows you to tweak the smoothing period, and is expressed in seconds. Setting it to 0 will result in completely unsmoothed frame times (the default)
		/// </remarks>
		...
		/// <summary>
		///   Get/Set the WorkQueue for processing background tasks. You are free to add new requests and handlers to this queue to process your custom background tasks using the shared thread pool. However, you must remember to assign yourself a new channel through which to process your tasks.
		/// </summary>
		/// <remarks>
		///   Root will delete this work queue at shutdown, so do not destroy it yourself.
		/// </remarks>
		...
		///</summary>
		///<remarks>
		///  You can use this method to create a SceneManager instance of a given specific type. You may know this type already, or you may have discovered it by looking at the results from <see
		///   cref="Root.GetSceneManagerMetaData" /> .
		///</remarks>
		...
		///</summary>
		///<remarks>
		///  Creates an instance of a <see cref="SceneManager" /> which supports the scene types identified in the parameter. If more than one type of SceneManager has been registered as handling that combination of scene types, in instance of the last one registered is returned.
		///</remarks>
		...
		/// </summary>
		/// <remarks>
		///   This method can only be called after a renderer has been selected with <see cref="Root.RenderSystem" /> , and it will initialize the selected rendering system ready for use.
		/// </remarks>
		...
		/// <summary>
		///   Internal method used for updating all <see cref="RenderTarget" /> objects (windows, renderable textures etc) which are set to auto-update.
		/// </summary>
		/// <remarks>
		///   You don't need to use this method if you're using Axiom's own internal rendering loop ( <see cref="Root.StartRendering" /> ). If you're running your own loop you may wish to call it to update all the render targets which are set to auto update ( <see
		///    cref="RenderTarget.IsAutoUpdated" /> ). You can also update individual <see cref="RenderTarget" /> instances using their own Update() method.
		/// </remarks>
		...
		///    name="RenderWindow.Update" /> with the 'swapBuffers' option set to false.
		/// </remarks>
		...
		/// </summary>
		/// <remarks>
		///   This method is only for internal use when you use the built-in rendering loop (Root.StartRendering). However, if you run your own rendering loop then you should call this method to ensure that FrameEvent handlers are notified of frame events; processes like texture animation and particle systems rely on this. <p /> This method takes an event object as a parameter, so you can specify the times yourself. If you are happy for the engine to automatically calculate the frame time for you, then call the other version of this method with no parameters.
		/// </remarks>
