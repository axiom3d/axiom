...
namespace Axiom.Core
{
	...
	public class PatchSurface
	{
		                           ...
		                           VisibleSide visibleSide )
		{
			...
			controlPointBuffer = Memory.PinObject( controlPointArray );

			this.type = type;
			controlWidth = width;
			controlHeight = height;
			controlCount = width*height;
		...
		/// </summary>
		/// <remarks>
		///   The VertexDeclaration of the vertex buffer must be identical to the one passed into <see
		///    cref="DefineSurface(Array, VertexDeclaration, int, int)" /> . In addition, there must be enough space in the buffer to accommodate the patch at full detail level; you should check <see
		///    cref="RequiredVertexCount" /> and <see cref="RequiredIndexCount" /> to determine this. This method does not create an internal mesh for this patch and so GetMesh will return null if you call it after building the patch this way.
		/// </remarks>
		                   ...
		                   int indexStart )
		{
			...
			var lockedBuffer = vertexBuffer.Lock( vertexOffset*declaration.GetVertexSize( 0 ),
			                                      requiredVertexCount*declaration.GetVertexSize( 0 ), BufferLocking.NoOverwrite );
		...
		protected void MakeTriangles()
		{
...
#endif
			{
				...
				var p32 = use32bitindexes
				          	? indexBuffer.Lock( indexOffset*sizeof ( int ), requiredIndexCount*sizeof ( int ),
				          	                    BufferLocking.NoOverwrite ).ToIntPointer()
				          	: null;
				var p16 = !use32bitindexes
				          	? indexBuffer.Lock( indexOffset*sizeof ( short ), requiredIndexCount*sizeof ( short ),
		...
		/// <summary>
		///   Based on a previous call to <see cref="DefineSurface(Array, VertexDeclaration, int, int)" /> , establishes the number of vertices required to hold this patch at the maximum detail level.
		/// </summary>
		/// <remarks>
		...
		/// <summary>
		///   Based on a previous call to <see cref="DefineSurface(Array, VertexDeclaration, int, int)" /> , establishes the number of indexes required to hold this patch at the maximum detail level.
		/// </summary>
		...
		/// <summary>
		///   Gets the bounds of this patch, only valid after calling <see cref="DefineSurface(Array, VertexDeclaration, int, int)" /> .
		/// </summary>
		...
		/// <summary>
		///   Gets the radius of the bounding sphere for this patch, only valid after <see
		///    cref="DefineSurface(Array, VertexDeclaration, int, int)" /> has been called.
		/// </summary>
		...
		/// </summary>
		/// <remarks>
		///   This method changes the proportionate detail level of the patch; since the U and V directions can have different subdivision levels, this property takes a single float value where 0 is the minimum detail (the control points) and 1 is the maximum detail level as supplied to the original call to <see
		///    cref="DefineSurface(Array, VertexDeclaration, int, int)" /> .
		/// </remarks>
