...
{
	/// <summary>
	///   This is a container class for sub render state class a render state is defined by the tsub render states that compose it. The user should use this interfact define globabl or per material custom behavior. I.E. in order to a pplhy per pixel to a specific material, one should implement a sub class of SubRenderState that performs a per pixel lighting model, get the render state of the target material, and add the custom sub class to it
	/// </summary>
	public class RenderState
	{
		...
		/// <summary>
		///   Gets the light count lightCount[0] defines the point light count. lightCount[1] defines the directional light count. lightCount[2] defines the spot light count
		/// </summary>
		...
		public bool LightCountAutoUpdate { get; set; }
	...
	internal class TargetRenderStateComparer : IComparer<SubRenderState>
	{
