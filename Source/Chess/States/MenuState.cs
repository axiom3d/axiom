...
namespace Chess.States 
{
	...
	public class MenuState: ChessState
	{
		...
		public override void Initialize()
		{
			...
			ChessApplication.Instance.MenuOverlay = (menuOverlay);
			selectedItem = 0;
		...
		{
			menuOverlay.Hide();
			ChessApplication.Instance.MenuOverlay = (null);
		}
		public override void FrameStarted(float dt)
		{
			// Slowly rotate level
			float Pi = (float)System.Math.PI;
			game.cameraWantedYaw += Axiom.Math.Utility.RadiansToDegrees(0.25f*dt);
			while (game.cameraWantedYaw > (2.0f*Pi))
				game.cameraWantedYaw -= Axiom.Math.Utility.RadiansToDegrees(2.0f*Pi);
		...
		protected virtual void OnSelected(int item) 
		{
