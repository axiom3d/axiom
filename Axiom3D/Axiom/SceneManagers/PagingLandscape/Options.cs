...
namespace Axiom.SceneManagers.PagingLandscape
{
    ...
    /// </summary>
    internal sealed class Options : Hashtable, IDisposable
    {
        ...
        public void Load( string filename )
        {
            ...
            if ( this.Vertex_Shadowed )
                this.Vertex_Instant_Colored = true;
            ...
            if ( val != string.Empty )
                this.Num_Renderables = long.Parse( val );

            val = this.getSetting( "IncrementRenderables" );
            if ( val != string.Empty )
                this.Num_Renderables_Increment = long.Parse( val );

            val = this.getSetting( "MaxNumTiles" );
            if ( val != string.Empty )
                this.Num_Tiles = long.Parse( val );

            val = this.getSetting( "IncrementTiles" );
            if ( val != string.Empty )
                this.Num_Tiles_Increment = long.Parse( val );
            ...
            if ( val != string.Empty )
                this.Num_Renderables_Loading = long.Parse( val );

            val = this.getSetting( "MaxAdjacentPages" );
            if ( val != string.Empty )
                this.Max_Adjacent_Pages = long.Parse( val );

            val = this.getSetting( "MaxPreloadedPages" );
            if ( val != string.Empty )
                this.Max_Preload_Pages = long.Parse( val );

            val = this.getSetting( "Height" );
            if ( val != string.Empty )
                this.World_Height = long.Parse( val );

            val = this.getSetting( "Width" );
            if ( val != string.Empty )
                this.World_Width = long.Parse( val );

            val = this.getSetting( "PageSize" );
            if ( val != string.Empty )
                this.PageSize = long.Parse( val );

            val = this.getSetting( "TileSize" );
            if ( val != string.Empty )
                this.TileSize = long.Parse( val );

            val = this.getSetting( "MaxRenderLevel" );
            if ( val != string.Empty )
                this.MaxRenderLevel = long.Parse( val );
