	...
	public class OpenGLRenderer : RenderSystem, IPlugin
	{
		...
		public override void Render(RenderOperation op)
		{
			...
			for(int i = 0; i < elements.Count; i++)
			{
				...
				Ext.glBindBufferARB(Gl.GL_ARRAY_BUFFER_ARB, vertexBuffer.GLBufferID);
		...
		{
			uint lightIndex = Gl.GL_LIGHT0 + (uint)index;
			...
			for (int i = 0; i < MAX_LIGHTS; i++)
			{
				if (lights[i] != null)
				{
					...
					if (lt.Type != LightType.Directional)
					{
						...
						f4vals[2] = vec.z;
						f4vals[3] = 1.0f;
						Gl.glLightfv(Gl.GL_LIGHT0 + (uint)i, Gl.GL_POSITION, f4vals);
					}
					// Direction (not needed for point lights)
					if (lt.Type != LightType.Point)
					{
						...
						f4vals[2] = vec.z;
						f4vals[3] = 0.0f;
						Gl.glLightfv(Gl.GL_LIGHT0 + (uint)i, Gl.GL_SPOT_DIRECTION, f4vals);
					}
				}
			}
		}
		...
		public override void SetTextureBlendMode(int stage, LayerBlendModeEx blendMode)
		{
			...
			Gl.glTexEnvi(Gl.GL_TEXTURE_ENV, Gl.GL_OPERAND0_RGB, (int)Gl.GL_SRC_COLOR);
			Gl.glTexEnvi(Gl.GL_TEXTURE_ENV, Gl.GL_OPERAND1_RGB, (int)Gl.GL_SRC_COLOR);
			Gl.glTexEnvi(Gl.GL_TEXTURE_ENV, Gl.GL_OPERAND2_RGB, (int)Gl.GL_SRC_COLOR);
			Gl.glTexEnvi(Gl.GL_TEXTURE_ENV, Gl.GL_OPERAND0_ALPHA, (int)Gl.GL_SRC_ALPHA);
			Gl.glTexEnvi(Gl.GL_TEXTURE_ENV, Gl.GL_OPERAND1_ALPHA, (int)Gl.GL_SRC_ALPHA);
			Gl.glTexEnvi(Gl.GL_TEXTURE_ENV, Gl.GL_OPERAND2_ALPHA, (int)Gl.GL_SRC_ALPHA);
