...
namespace Axiom.MathLib
{
    ...
    public struct Vector3 : ICustomTypeDescriptor, IParsable
    {
        ...
        {
            if ( !float.IsNaN( minX ) && x < minX )
                this.x = x;
            if ( !float.IsNaN( minY ) && y < minY )
                this.y = y;
            if ( !float.IsNaN( minZ ) && z < minZ )
                this.z = z;
        }
        public void KeepGreaterThan( float maxX, float maxY, float maxZ )
        {
            if ( !float.IsNaN( maxX ) && x > maxX )
                this.x = x;
            if ( !float.IsNaN( maxY ) && y > maxY )
                this.y = y;
            if ( !float.IsNaN( maxZ ) && z > maxZ )
                this.z = z;
        }
